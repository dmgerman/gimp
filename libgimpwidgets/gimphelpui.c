...
name|g_print
argument_list|(
literal|"FIXME: gimp_help_connect_help_accel\n"
argument_list|)
expr_stmt|;
comment|/*   if (! gtk_signal_lookup ("tips_query", G_TYPE_FROM_INSTANCE (widget)))     {       gtk_object_class_user_signal_new (GTK_OBJECT (widget)->klass, 					"tips_query", 					GTK_RUN_LAST, 					g_cclosure_marshal_VOID__VOID, 					G_TYPE_NONE, 					0, 					NULL);        gtk_object_class_user_signal_new (GTK_OBJECT (widget)->klass, 					"help", 					GTK_RUN_LAST, 					g_cclosure_marshal_VOID__VOID, 					G_TYPE_NONE, 					0, 					NULL);     }   */
...
directive|if
literal|0
block|accel_group = gtk_accel_group_new ();
...
block|gtk_accel_group_add (accel_group, GDK_F1, 0, 0, 		       GTK_OBJECT (widget), "help");   gtk_accel_group_add (accel_group, GDK_F1, GDK_SHIFT_MASK, 0, 		       GTK_OBJECT (widget), "tips_query");    gtk_accel_group_attach (accel_group, GTK_OBJECT (widget));
endif|#
directive|endif
block|}
end_function
