...
comment|/*  Check - background colour only works for non-interpolated transforms? */
...
comment|/* Actually, no. Relabel them in your head, if you must  */
name|tx
operator|=
name|xinc
operator|*
...
condition|)
block|{
comment|/*  ttx& tty are the coordinates of the point in the original                *  selection's floating buffer.  Make sure they're within bounds                */
name|itx
operator|=
name|floor
argument_list|(
...
comment|/* (which runs from itx - 1 to itx + 2, same in y) */
if|if
condition|(
operator|(
name|itx
...
name|xy_i
decl_stmt|;
comment|/*  get the fractional error  */
name|double
name|dx
decl_stmt|,
name|dy
...
name|ity
expr_stmt|;
comment|/* setup indices and clip flags */
comment|/* now correctly handles very small images (why?) */
for|for
control|(
name|xy_i
operator|=
...
expr_stmt|;
block|}
comment|/* ugly, ugly code */
comment|/* clip x to left hand edge */
if|if
condition|(
operator|(
name|itx
...
block|}
block|}
comment|/* clip x to right hand edge */
if|if
condition|(
operator|(
name|itx
