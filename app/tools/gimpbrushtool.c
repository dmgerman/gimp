...
block|if (paint_tool ==&non_gui_paint_tool)     {       paint_tool->cur_coords.pressure = 1.0;       paint_tool->cur_coords.xtilt    = 0.5;       paint_tool->cur_coords.ytilt    = 0.5;       paint_tool->cur_coords.wheel    = 0.5;        paint_tool->start_coords = paint_tool->cur_coords;       paint_tool->last_coords  = paint_tool->cur_coords;     }
endif|#
directive|endif
...
begin_comment
unit|pixel_region_init (&srcPR, canvas_tiles, 		     canvas_buf->x, canvas_buf->y, 		     canvas_buf->width, canvas_buf->height, TRUE);    x = (gint) floor (paint_tool->cur_coords.x) - (brush_mask->width>> 1);   y = (gint) floor (paint_tool->cur_coords.y) - (brush_mask->height>> 1);   xoff = (x< 0) ? -x : 0;   yoff = (y< 0) ? -y : 0;    maskPR.bytes     = 1;   maskPR.x         = 0;   maskPR.y         = 0;   maskPR.w         = srcPR.w;   maskPR.h         = srcPR.h;   maskPR.rowstride = maskPR.bytes * brush_mask->width;   maskPR.data      = mask_buf_data (brush_mask) + yoff * maskPR.rowstride + xoff * maskPR.bytes;
