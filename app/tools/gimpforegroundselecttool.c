...
typedef|typedef
struct|struct
DECL|struct|__anon2784f1440108
block|{
DECL|member|width
name|gint
name|width
...
directive|if
literal|0
block|if (fg_select->state)     g_warning ("%s: state should be NULL at this point", G_STRLOC);
endif|#
directive|endif
if|if
condition|(
...
directive|if
literal|0
block|if (fg_select->state)           {             gimp_drawable_foreground_extract_siox_done (fg_select->state);             fg_select->state = NULL;           }
endif|#
directive|endif
name|tool
operator|->
...
name|mask
condition|)
comment|/* dunno if that's the right condition */
block|{
switch|switch
condition|(
name|kevent
...
directive|if
literal|0
block|if (fg_select->state)         gimp_drawable_foreground_extract_siox (GIMP_DRAWABLE (mask),                                                fg_select->state,                                                fg_select->refinement,                                                options->smoothness,                                                options->sensitivity,                                                ! options->contiguous,                                                GIMP_PROGRESS (fg_select));
endif|#
directive|endif
name|fg_select
operator|->
...
directive|if
literal|0
block|if (fg_select->state)         g_warning ("state should be NULL here");        fg_select->state =         gimp_drawable_foreground_extract_siox_init (drawable,                                                     x1, y1, x2 - x1, y2 - y1);
endif|#
directive|endif
block|}
name|gimp_foreground_select_tool_set_mask
...
directive|if
literal|0
block|if (strcmp (pspec->name, "smoothness") == 0)     {       refinement = SIOX_REFINEMENT_CHANGE_SMOOTHNESS;     }   else if (strcmp (pspec->name, "contiguous") == 0)     {       refinement = SIOX_REFINEMENT_CHANGE_MULTIBLOB;     }   else if (g_str_has_prefix (pspec->name, "sensitivity"))     {       refinement = SIOX_REFINEMENT_CHANGE_SENSITIVITY;     }
endif|#
directive|endif
if|if
condition|(
