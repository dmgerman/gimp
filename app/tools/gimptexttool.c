...
directive|ifdef
name|TEXT_TOOL_HACK
comment|/*  we need this crap because we need some object to call    *  gtk_binding_set_activate() activate with. It takes a GtkObject    *  instead of a GObject. So instead of adding the needed binding    *  signals to GimpTextTool, we abuse a GtkTextView, which has all    *  the needed signals anyway, and connect to its signals. Puke!    */
name|text_tool
operator|->
