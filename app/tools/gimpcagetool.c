...
name|FALSE
expr_stmt|;
comment|//gimp_cage_tool_prepare_preview (ct, display);
...
name|NULL
expr_stmt|;
comment|//g_object_unref (ct->image_map);
comment|//ct->image_map = NULL;
block|}
end_function
...
condition|)
block|{
comment|//gimp_cage_config_reverse_cage_if_needed (ct->config);
comment|//gimp_cage_tool_compute_coef (ct, display);
block|}
name|gimp_draw_tool_resume
argument_list|(
name|GIMP_DRAW_TOOL
...
expr_stmt|;
block|}
comment|/*  {     GeglRectangle rect = {0, 0, 300, 300};     gimp_image_map_apply (ct->image_map,&rect);   }*/
name|gimp_draw_tool_resume
argument_list|(
name|draw_tool
argument_list|)
...
directive|endif
comment|/*gimp_drawable_apply_operation (drawable, progress, _("Cage transform"),                                    debug, TRUE);*/
name|gegl_node_process
argument_list|(
name|output
argument_list|)
