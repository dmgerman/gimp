...
directive|if
literal|0
block|if (options->alternate)     {       gdouble *x0, *x1, *y0, *y1;       gboolean moveedge = FALSE;        switch (function)         {         case TRANSFORM_HANDLE_W:           x0 = x[0]; y0 = y[0];           x1 = x[2]; y1 = y[2];           moveedge = TRUE;           break;          case TRANSFORM_HANDLE_S:           x0 = x[2]; y0 = y[2];           x1 = x[3]; y1 = y[3];           moveedge = TRUE;           break;          case TRANSFORM_HANDLE_N:           x0 = x[0]; y0 = y[0];           x1 = x[1]; y1 = y[1];           moveedge = TRUE;           break;          case TRANSFORM_HANDLE_E:           x0 = x[1]; y0 = y[1];           x1 = x[3]; y1 = y[3];           moveedge = TRUE;           break;          case TRANSFORM_HANDLE_NW:           *x[0] += diff_x;           *y[0] += diff_y;           return;          case TRANSFORM_HANDLE_NE:           *x[1] += diff_x;           *y[1] += diff_y;           return;          case TRANSFORM_HANDLE_SW:           *x[2] += diff_x;           *y[2] += diff_y;           return;          case TRANSFORM_HANDLE_SE:           *x[3] += diff_x;           *y[3] += diff_y;           return;          default:           break;         }       if (moveedge)         {           *x0 += diff_x;           *x1 += diff_x;           *y0 += diff_y;           *y1 += diff_y;           return;         }     }    switch (function)     {     case TRANSFORM_HANDLE_NW:     case TRANSFORM_HANDLE_NE:     case TRANSFORM_HANDLE_SW:     case TRANSFORM_HANDLE_SE:     {       GimpVector2 m = { .x = transform_tool->curx,   .y = transform_tool->cury };       GimpVector2 p = { .x = transform_tool->mousex, .y = transform_tool->mousey };       GimpVector2 c = { .x = *pivot_x,               .y = *pivot_y };       gdouble angle = calcangle (vectorsubtract (m, c), vectorsubtract (p, c));       for (i = 0; i< 4; i++) {         p.x = px[i]; p.y = py[i];         m = vectoradd (c, rotate2d (vectorsubtract (p, c), angle));         *x[i] = m.x;         *y[i] = m.y;       }       return;     }     case TRANSFORM_HANDLE_CENTER:       if (constrain) {         diff_y = diff_x;       }       *x[0] += diff_x;       *y[0] += diff_y;       *x[1] += diff_x;       *y[1] += diff_y;       *x[2] += diff_x;       *y[2] += diff_y;       *x[3] += diff_x;       *y[3] += diff_y;       break;      case TRANSFORM_HANDLE_PIVOT:       *pivot_x += diff_x;       *pivot_y += diff_y;       break;      case TRANSFORM_HANDLE_E:     case TRANSFORM_HANDLE_W:       horizontal = TRUE;     case TRANSFORM_HANDLE_N:     case TRANSFORM_HANDLE_S:       if (!constrain)         {           for (i = 0; i< 4; i++)             {               if (horizontal)                 *x[i] = *pivot_x + (*pivot_x-transform_tool->curx)/(*pivot_x-transform_tool->mousex)*(px[i]-*pivot_x);               else                 *y[i] = *pivot_y + (*pivot_y-transform_tool->cury)/(*pivot_y-transform_tool->mousey)*(py[i]-*pivot_y);             }         } else {           GimpVector2 m = { .x = transform_tool->curx,   .y = transform_tool->cury };           GimpVector2 p = { .x = transform_tool->mousex, .y = transform_tool->mousey };           GimpVector2 c = { .x = *pivot_x,               .y = *pivot_y };           gdouble onorm = 1./norm (vectorsubtract (c, p));           gdouble distance = norm (vectorsubtract (c, m)) * onorm;           for (i = 0; i< 4; i++) {             p.x = px[i]; p.y = py[i];             m = vectoradd (c, scalemult (vectorsubtract (p, c), distance));             *x[i] = m.x;             *y[i] = m.y;           }         }       break;      default:       break;     }
endif|#
directive|endif
block|}
end_function
