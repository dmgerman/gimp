...
block|{
comment|/* when the keep aspect transformation constraint is enabled, all            * four corners shall translate to keep all sides at constant angles. */
name|GimpVector2
name|mtl
decl_stmt|,
name|mtr
...
name|nor
decl_stmt|;
comment|/* Where the two points on the moved side would move usually */
name|mtl
operator|=
name|vectoradd
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Intersect the line formed by mtl-mtr with the two diagonals in            * the pre-interaction transform, this is where these two corners            * will be */
name|ntl
operator|=
name|lineintersect
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Extend two lines from the new position of our corners along the            * direction of the respective side (ol-tl and or-tr are the two sides            * going out perpendicular to the side being interacted with) */
name|tmpl
operator|=
name|vectoradd
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Now intersect the lines tmpl-ntl with one diagonal, and tmpr-ntr with            * the other diagonal, giving the points of the corners of the side opposite            * the one that was interacted with */
name|nol
operator|=
name|lineintersect
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* We have all the new points, assign them to the real vars */
name|tl
operator|=
name|ntl
expr_stmt|;
