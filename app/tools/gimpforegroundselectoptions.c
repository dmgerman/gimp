...
directive|if
literal|0
block|GIMP_CONFIG_INSTALL_PROP_INT (object_class, PROP_SMOOTHNESS,                                 "smoothness",                                 N_("Smaller values give a more accurate "                                   "selection border but may introduce holes "                                   "in the selection"),                                 0, 8, SIOX_DEFAULT_SMOOTHNESS,                                 GIMP_PARAM_STATIC_STRINGS);
endif|#
directive|endif
...
directive|if
literal|0
block|scale = gimp_prop_hscale_new (config, "smoothness", 0.1, 1.0, 0);   gtk_scale_set_value_pos (GTK_SCALE (scale), GTK_POS_RIGHT);   gimp_table_attach_aligned (GTK_TABLE (table), 0, 0,                              _("Smoothing:"), 0.0, 0.5, scale, 2, FALSE);
endif|#
directive|endif
comment|/*  mask color */
name|menu
...
literal|0
comment|/*  granularity  */
block|frame = gimp_prop_expander_new (config, "expanded", _("Color Sensitivity"));   gtk_box_pack_start (GTK_BOX (vbox), frame, FALSE, FALSE, 0);   gtk_widget_show (frame);    inner_frame = gimp_frame_new ("<expander>");   gtk_container_add (GTK_CONTAINER (frame), inner_frame);   gtk_widget_show (inner_frame);    table = gtk_table_new (3, 3, FALSE);   gtk_table_set_col_spacings (GTK_TABLE (table), 2);   gtk_table_set_row_spacings (GTK_TABLE (table), 2);   gtk_container_add (GTK_CONTAINER (inner_frame), table);   gtk_widget_show (table);    gimp_prop_opacity_entry_new (config, "sensitivity-l",                                GTK_TABLE (table), 0, row++, "L");    gimp_prop_opacity_entry_new (config, "sensitivity-a",                                GTK_TABLE (table), 0, row++, "a");    gimp_prop_opacity_entry_new (config, "sensitivity-b",                                GTK_TABLE (table), 0, row++, "b");
endif|#
directive|endif
return|return
name|vbox
