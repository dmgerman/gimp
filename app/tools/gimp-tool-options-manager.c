...
name|global_props
expr_stmt|;
comment|/*  if the active tool uses any of the newly enabled global props,    *  copy them to the user_context/global_paint_options first, so    *  what the user currently uses becomes the new global values    */
if|if
condition|(
name|manager
operator|->
...
expr_stmt|;
block|}
comment|/*  then, copy the newly enabled global props to all tool options,    *  and disconnect the newly disabled ones from the user context    */
for|for
control|(
name|list
operator|=
