...
comment|/*FIX      if (!edge_map_blocks) 	allocate_edge_map_blocks (BLOCK_WIDTH, BLOCK_HEIGHT, 				  gimage_width (gdisp->gimage), 				  gimage_height (gdisp->gimage)); 				  */
name|iscissors
operator|->
name|num_segs
operator|=
...
argument_list|)
expr_stmt|;
comment|/*FIXkinks[i].x = BOUNDS (x, 0, (gimage_width (gdisp->gimage) - 1));       kinks[i].y = BOUNDS (y, 0, (gimage_height (gdisp->gimage) - 1));*/
...
literal|0
expr_stmt|;
comment|/*FIXx1 = gimage_width (gdisp->gimage);   y1 = gimage_height (gdisp->gimage);*/
...
control|)
block|{
comment|/*FIX      x = BOUNDS (pts[i].x - LOCALIZE_RADIUS, 0, 		  gimage_width (gdisp->gimage));       y = BOUNDS (pts[i].y - LOCALIZE_RADIUS, 0, 		  gimage_height (gdisp->gimage));       w = BOUNDS (pts[i].x + LOCALIZE_RADIUS, 0, 		  gimage_width (gdisp->gimage));       h = BOUNDS (pts[i].y + LOCALIZE_RADIUS, 0, 		  gimage_height (gdisp->gimage)); 		  */
name|w
operator|-=
name|x
expr_stmt|;
...
comment|/*FIXx1 = BOUNDS (x, 0, gimage_width (gimage));   y1 = BOUNDS (y, 0, gimage_height (gimage));   x2 = BOUNDS (x + w, 0, gimage_width (gimage));   y2 = BOUNDS (y + h, 0, gimage_height (gimage));*/
name|width
operator|=
name|x2
operator|-
...
operator|)
expr_stmt|;
comment|/*  Set the gimage up as the source pixel region  */
comment|/* FIX srcPR.bytes = gimage_bytes (gimage);*/
name|srcPR
operator|.
name|w
operator|=
...
name|height
expr_stmt|;
comment|/* FIX srcPR.rowstride = gimage_width (gimage) * gimage_bytes (gimage);*/
comment|/*FIXsrcPR.data = gimage_data (gimage) + y1 * srcPR.rowstride + x1 * srcPR.bytes;*/
...
literal|128
expr_stmt|;
comment|/*FIX	  for (b = 1; b< gimage_bytes (gimage); b++) 	    { 	      if (abs (dh[b] - 128)> abs (hmax)) hmax = dh[b] - 128; 	      if (abs (dv[b] - 128)> abs (vmax)) vmax = dv[b] - 128; 	    }*/
...
expr_stmt|;
block|}
comment|/*  show the edge buffer  */
comment|/*  temp_buf_to_gdisplay (edge_buf);*/
block|}
end_function
...
name|gimage_ptr
expr_stmt|;
comment|/*FIXwidth = gimage_width (gimage);*/
comment|/*FIXheight = gimage_height (gimage);*/
name|startx
operator|=
name|x
expr_stmt|;
...
specifier|static
name|void
DECL|function|CR_convert_line (link_ptr * scanlines,int x1,int y1,int x2,int y2)
name|CR_convert_line
parameter_list|(
name|link_ptr
modifier|*
...
specifier|static
name|link_ptr
DECL|function|CR_insert_in_list (link_ptr list,int x)
name|CR_insert_in_list
parameter_list|(
name|link_ptr
name|list
