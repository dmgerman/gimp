...
begin_comment
comment|/*  cost map blocks variables  */
end_comment

begin_decl_stmt
DECL|variable|cost_map_blocks
specifier|static
name|TempBuf
modifier|*
modifier|*
name|cost_map_blocks
init|=
name|NULL
decl_stmt|;
end_decl_stmt
...
begin_comment
comment|/*  static unsigned char  cost_conv0 [CONV_WIDTH * CONV_HEIGHT * 3];*/
end_comment

begin_decl_stmt
DECL|variable|sentinel4
specifier|static
name|guint32
name|sentinel4
init|=
...
if|else if (iscissors->connected&& iscissors->region&& 	       gregion_point_inside (iscissors->region, iscissors->x, iscissors->y)) 	{
comment|/*  Undraw the curve  */
if|tool->state = INACTIVE; 	  iscissors->draw = DRAW_CURVE; 	  draw_core_stop (iscissors->core, tool);  	  replace = 0; 	  if (bevent->state& ShiftMask) 	    op = ADD; 	  else if (bevent->state& ControlMask) 	    op = SUB; 	  else 	    { 	      op = ADD; 	      replace = 1; 	    }  	  if (replace) 	    selection_clear (gdisp->select, gdisp_ptr); 	  else 	    gimage_mask_undo (gdisp->gimage);  	  if (((SelectionOptions *) iscissors_options)->feather) 	    channel_feather (iscissors->mask, 			     gimage_get_mask (gdisp->gimage), 			     ((SelectionOptions *) iscissors_options)->feather_radius, 			     ((SelectionOptions *) iscissors_options)->feather_radius, 			     op, 0, 0); 	  else 	    channel_combine_mask (gimage_get_mask (gdisp->gimage), 				  iscissors->mask, op, 0, 0);  	  iscissors_reset (iscissors); 	   	  selection_start (gdisp->select, 0, True);  	}
endif|#
directive|endif
comment|/* 0 */
...
block|if (iscissors->connected)     iscissors_convert (iscissors, gdisp_ptr);
endif|#
directive|endif
comment|/* 0 */
block|}
...
name|printf
argument_list|(
literal|"1154: g_ptr_array_free (curve->points);\n"
argument_list|)
expr_stmt|;
name|g_ptr_array_free
argument_list|(
