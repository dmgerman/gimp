...
specifier|static
name|void
name|gimp_dialog_factory_config_notify
parameter_list|(
name|GimpDialogFactory
modifier|*
name|factory
...
name|GtkWidget
modifier|*
DECL|function|gimp_dialog_factory_find_widget (GimpDialogFactory * factory,const gchar * identifiers)
...
name|GtkWidget
modifier|*
DECL|function|gimp_dialog_factory_dialog_new_internal (GimpDialogFactory * factory,GdkMonitor * monitor,GimpContext * context,GimpUIManager * ui_manager,GtkWidget * parent,const gchar * identifier,gint view_size,gboolean return_existing,gboolean present,gboolean create_containers)
...
name|g_warning
argument_list|(
literal|"%s: GimpDialogFactory is a dock factory entry "
literal|"but constructor for \"%s\" did not return a "
literal|"GimpDock"
argument_list|,
name|G_STRFUNC
...
name|GtkWidget
modifier|*
DECL|function|gimp_dialog_factory_dialog_new (GimpDialogFactory * factory,GdkMonitor * monitor,GimpUIManager * ui_manager,GtkWidget * parent,const gchar * identifier,gint view_size,gboolean present)
name|gimp_dialog_factory_dialog_new
parameter_list|(
name|GimpDialogFactory
modifier|*
...
name|GtkWidget
modifier|*
DECL|function|gimp_dialog_factory_dialog_raise (GimpDialogFactory * factory,GdkMonitor * monitor,GtkWidget * parent,const gchar * identifiers,gint view_size)
...
argument_list|)
expr_stmt|;
comment|/*  If the identifier is a list, try to find a matching dialog and    *  raise it. If there's no match, use the first list item.    *    *  (we split the identifier list manually here because we must pass    *  a single identifier, not a list, to new_internal() below)    */
name|ids
operator|=
name|g_strsplit
argument_list|(
...
begin_comment
comment|/**  * gimp_dialog_factory_position_dialog:  * @factory:  * @identifier:  * @dialog:  * @monitor:  *  * We correctly position all newly created dialog via  * gimp_dialog_factory_add_dialog(), but some dialogs (like various  * color dialogs) are never destroyed but created only once per  * session. On re-showing, whatever window managing magic kicks in and  * the dialog sometimes goes where it shouldn't.  *  * This function correctly positions a dialog on re-showing so it  * appears where it was before it was hidden.  *  * See https://gitlab.gnome.org/GNOME/gimp/issues/1093  **/
end_comment

begin_function
name|void
DECL|function|gimp_dialog_factory_position_dialog (GimpDialogFactory * factory,const gchar * identifier,GtkWidget * dialog,GdkMonitor * monitor)
...
name|g_warning
argument_list|(
literal|"%s: no session info found for \"%s\""
argument_list|,
name|G_STRFUNC
argument_list|,
name|identifier
...
name|g_warning
argument_list|(
literal|"%s: session info for \"%s\" is for a different widget"
argument_list|,
name|G_STRFUNC
argument_list|,
name|identifier
...
specifier|static
name|void
DECL|function|gimp_dialog_factory_config_notify (GimpDialogFactory * factory,GParamSpec * pspec,GimpGuiConfig * config)
name|gimp_dialog_factory_config_notify
parameter_list|(
name|GimpDialogFactory
modifier|*
...
name|state
decl_stmt|;
comment|/* Make sure the state and config are in sync */
if|if
condition|(
name|config
operator|->
