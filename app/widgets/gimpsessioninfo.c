...
block|,
DECL|enumerator|SESSION_INFO_MONITOR
name|SESSION_INFO_MONITOR
block|,
...
name|dialog
decl_stmt|;
DECL|typedef|GimpRestoreDocksData
block|}
...
specifier|static
name|gboolean
name|gimp_session_info_restore_docks
parameter_list|(
name|GimpRestoreDocksData
modifier|*
name|data
...
specifier|static
name|gint
DECL|function|monitor_number (GdkMonitor * monitor)
name|monitor_number
parameter_list|(
name|GdkMonitor
modifier|*
...
name|n_monitors
init|=
name|gdk_display_get_n_monitors
argument_list|(
name|display
argument_list|)
decl_stmt|;
...
name|TRUE
expr_stmt|;
comment|/*  the screen number is optional, and obsolete  */
if|if
condition|(
name|g_scanner_peek_next_token
argument_list|(
...
specifier|static
name|gboolean
DECL|function|gimp_session_info_restore_docks (GimpRestoreDocksData * data)
name|gimp_session_info_restore_docks
parameter_list|(
name|GimpRestoreDocksData
modifier|*
...
begin_function
name|void
DECL|function|gimp_session_info_restore (GimpSessionInfo * info,GimpDialogFactory * factory,GdkMonitor * monitor)
...
argument_list|)
expr_stmt|;
comment|/* In single-window mode, gimp_session_managed_set_aux_info()    * will set the size of the dock areas at the sides. If we don't    * wait for those areas to get their size-allocation, we can't    * properly restore the docks inside them, so do that in an idle    * callback.    */
comment|/* Objects are unreffed again in the callback */
name|data
operator|=
name|g_slice_new0
argument_list|(
...
begin_comment
comment|/**  * gimp_session_info_apply_geometry:  * @info:  * @monitor:  * @current_monitor:  *  * Apply the geometry stored in the session info object to the  * associated widget.  **/
end_comment

begin_function
name|void
DECL|function|gimp_session_info_apply_geometry (GimpSessionInfo * info,GdkMonitor * current_monitor,gboolean apply_stored_monitor)
...
argument_list|)
expr_stmt|;
comment|/* Don't write negative values to the sessionrc, they are        * interpreted as relative to the right, respective bottom edge        * of the display.        */
name|info
operator|->
name|p
operator|->
...
argument_list|)
expr_stmt|;
comment|/* Always store window coordinates relative to the monitor */
name|info
operator|->
name|p
operator|->
...
begin_comment
comment|/**  * gimp_session_info_get_info_with_widget:  * @info:  * @widget: #GtkWidget to use  *  * Temporarily sets @widget on @info and calls  * gimp_session_info_get_info(), then restores the old widget that was  * set.  **/
end_comment

begin_function
name|void
DECL|function|gimp_session_info_get_info_with_widget (GimpSessionInfo * info,GtkWidget * widget)
