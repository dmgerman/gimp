...
directive|if
literal|0
block|if (! pixbuf)     {       GFile     *file;       GFileInfo *file_info;        file = g_file_new_for_uri (gimp_object_get_name (imagefile));       file_info = g_file_query_info (file, "standard::icon", 0, NULL, NULL);        if (file_info)         {           GIcon *icon = g_file_info_get_icon (file_info);            if (icon)             {               GtkIconInfo *info;                info = gtk_icon_theme_lookup_by_gicon (icon_theme, icon, size, 0);                if (info)                 pixbuf = gtk_icon_info_load_icon (info, NULL);             }           else             {
ifdef|#
directive|ifdef
name|GIMP_UNSTABLE
block|g_printerr ("no icon for: %s\n",                           gimp_object_get_name (imagefile));
endif|#
directive|endif
block|}            g_object_unref (file_info);         }        g_object_unref (file);     }
endif|#
directive|endif
if|if
condition|(
