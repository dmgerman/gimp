...
directive|if
literal|0
block|gimp_item_factory_update (gimp_editor->item_factory,                             gimp_editor->popup_data);    menu =     gtk_item_factory_get_widget (GTK_ITEM_FACTORY (gimp_editor->item_factory),                                  path);    if (menu)     gtk_menu_popup (GTK_MENU (menu), NULL, NULL,                     gimp_tool_options_editor_menu_pos, button,                     0, GDK_CURRENT_TIME);
endif|#
directive|endif
block|}
end_function
...
directive|if
literal|0
block|GtkItemFactory *item_factory;       GtkWidget      *item;        item_factory = GTK_ITEM_FACTORY (GIMP_EDITOR (editor)->item_factory);        item = gtk_item_factory_get_widget (item_factory,                                           "/Save Options to/New Entry...");        if (item)         gtk_widget_activate (item);
endif|#
directive|endif
block|}
block|}
