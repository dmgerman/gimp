...
directive|if
literal|0
block|target_list = gtk_drag_source_get_target_list (widget);    if (target_list)     {       gtk_target_list_add_table (target_list,&dnd_data->target_entry, 1);     }   else     {       target_list = gtk_target_list_new (&dnd_data->target_entry, 1);        gtk_drag_source_set_target_list (widget, target_list);       gtk_target_list_unref (target_list);     }
endif|#
directive|endif
block|}
end_function
...
directive|if
literal|0
block|target_list = gtk_drag_source_get_target_list (widget);    if (target_list)     {       GdkAtom atom;        atom = gdk_atom_intern (dnd_data->target_entry.target, TRUE);        if (atom != GDK_NONE)         gtk_target_list_remove (target_list, atom);     }
endif|#
directive|endif
block|}
end_function
