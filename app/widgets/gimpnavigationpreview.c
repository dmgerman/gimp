...
condition|)
block|{
comment|// change_scale (gdisp, GIMP_ZOOM_IN);
block|}
else|else
block|{
comment|/* 	  GtkAdjustment *adj = 	    (bevent->state& GDK_CONTROL_MASK) ? 	    gdisp->hsbdata : gdisp->vsbdata; 	  gfloat new_value = adj->value - adj->page_increment / 2; 	  new_value = 	    CLAMP (new_value, adj->lower, adj->upper - adj->page_size); 	  gtk_adjustment_set_value (adj, new_value); 	  */
block|}
break|break;
case|case
literal|5
...
condition|)
block|{
comment|// change_scale (gdisp, GIMP_ZOOM_OUT);
block|}
else|else
block|{
comment|/* 	  GtkAdjustment *adj = 	    (bevent->state& GDK_CONTROL_MASK) ? 	    gdisp->hsbdata : gdisp->vsbdata; 	  gfloat new_value = adj->value + adj->page_increment / 2; 	  new_value = CLAMP (new_value, 			     adj->lower, adj->upper - adj->page_size); 	  gtk_adjustment_set_value (adj, new_value); 	  */
