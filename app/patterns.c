...
specifier|static
name|link_ptr
DECL|function|insert_pattern_in_list (list,pattern)
name|insert_pattern_in_list
parameter_list|(
name|list
parameter_list|,
...
name|val
decl_stmt|;
comment|/* Insert the item in the list */
if|if
condition|(
name|list
condition|)
...
name|data
expr_stmt|;
comment|/* do the comparison needed for the insertion sort */
name|val
operator|=
name|strcmp
argument_list|(
...
condition|)
block|{
comment|/* this is the place the item goes */
comment|/* Insert the item into the list. We'll have to create 	       *  a new link and then do a little insertion. 	       */
name|new_link
operator|=
name|alloc_list
argument_list|()
...
return|;
block|}
comment|/* Advance to the next item in the list. 	   */
name|prev
operator|=
name|tmp
expr_stmt|;
...
comment|/* There are no items in the pattern list, so we'll just start      *  one right now.      */
name|list
operator|=
name|add_to_list
argument_list|(
...
begin_function
name|GPatternP
DECL|function|get_pattern_by_index (index)
