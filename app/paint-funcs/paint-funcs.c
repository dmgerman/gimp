...
literal|0
expr_stmt|;
comment|/* Initialize accum to the first pixel slice.  As there is no partial          pixel at start, that value is 0.  */
name|accum
operator|=
literal|0.0
expr_stmt|;
...
name|old_width
expr_stmt|;
comment|/* We've accumulated a whole pixel where just a slice of it was              needed.  Subtract it now.  */
name|slice
operator|=
operator|(
name|srcpos
...
directive|if
literal|0
comment|/* Sanity check: srcp should point to the next-to-last position.  */
block|if (!(srcp - src - b == old_width * bytes))         g_warning ("Assertion (srcp - src - b == old_width * bytes) failed.");
endif|#
directive|endif
block|}
block|}
