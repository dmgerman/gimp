...
name|pr
decl_stmt|;
comment|/*      g_print ("CR: %d,%d (%dx%d) -> %d,%d (%dx%d) :: [%d] [%d]\n", 	       src->x, 	       src->y, 	       src->w, 	       src->h, 	       dest->x, 	       dest->y, 	       dest->w, 	       dest->h, 	       src->tiles->levels->width, 	       dest->tiles->levels->width);fflush(stdout);*/
comment|/* If we're copying from the same offset in each drawable,    *  and the PixelRegions are the same size, and the drawables are    *  the same size, we can do a quick'n'cheap Copy-On-Write.  Otherwise    *  we have to do the copy the old fashioned way.    *    * For COW the PixelRegions must also be of the same dimensions as the    *  tilemanager level, for reasons of simplicity.  Future work    *  may remove this restriction.    */
if|#
directive|if
literal|0
block|printf("SPR[ D%p T%p R%d O%d,%d S%d,%d B%d d%d P%d ]\n", 	 src->data, src->tiles, src->rowstride, src->x, 	 src->y, src->w, src->h, src->bytes, src->dirty, 	 src->process_count);   printf("DPR[ D%p T%p R%d O%d,%d S%d,%d B%d d%d P%d ]\n", 	 dest->data, dest->tiles, dest->rowstride, dest->x, 	 dest->y, dest->w, dest->h, dest->bytes, dest->dirty, 	 dest->process_count);   fflush(stdout);
endif|#
directive|endif
if|if
condition|(
...
return|return;
block|}
comment|/* else 'The old-fashioned way.'    */
for|for
control|(
name|pr
operator|=
