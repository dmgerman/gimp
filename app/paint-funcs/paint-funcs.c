...
begin_function
name|void
DECL|function|map_to_color (guint src_type,const guchar * cmap,const guchar * src,guchar * rgb)
name|map_to_color
parameter_list|(
name|guint
name|src_type
...
control|)
block|{
comment|/* This algorithm calculates the weighted average of pixel data that          each output pixel must receive, taking into account that it always          scales down, i.e. there are more than one input pixels per each          output pixel.           source and destp are the data pointers, position is the index to          the split point, mant is the partial pixel data, tmp is where the          output value is calculated, accum is used to accumulate only whole          pixels, max is the number of whole pixels (if any).       */
comment|/* Both input and output data are interleaved.  Calculate the pointer          to each.  */
name|source
operator|=
operator|&
name|src
...
literal|1
expr_stmt|;
comment|/*  Load the first partial pixel data with one whole input pixel.  */
name|mant
operator|=
operator|*
name|source
...
operator|--
expr_stmt|;
comment|/* Get all possible whole pixels.  */
while|while
condition|(
name|max
condition|)
...
name|mant
expr_stmt|;
comment|/* Add up all the whole pixels and the                                       previous partial pixel.  */
name|mant
operator|=
operator|(
name|position
...
name|source
expr_stmt|;
comment|/* Calculate the last partial pixel.  */
name|tmp
operator|+=
name|mant
expr_stmt|;
comment|/* Add the last chunk to final result.  */
name|tmp
operator|*=
name|inv_step
expr_stmt|;
comment|/* Scale the pixel down (average.) */
name|mant
operator|=
operator|*
name|source
...
name|mant
expr_stmt|;
comment|/* Take the remaining pixel data.  */
operator|*
name|destp
operator|=
name|tmp
expr_stmt|;
comment|/* Store the final result.  */
name|destp
operator|+=
name|bytes
expr_stmt|;
comment|/* Advance to the next output pixel.  */
name|position
operator|+=
name|step
expr_stmt|;
comment|/* Advance to the next pixel position.  */
block|}
block|}
block|}
end_function
...
begin_function
name|void
DECL|function|combine_mask_and_region (PixelRegion * src,PixelRegion * mask,guint opacity)
