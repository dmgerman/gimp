...
specifier|static
name|void
DECL|function|draw_segments (PixelRegion * destPR,BoundSeg * bs,int num_segs,int off_x,int off_y,int opacity)
name|draw_segments
parameter_list|(
name|PixelRegion
modifier|*
...
parameter_list|)
define|\
value|if (new_alpha == 0 || src2_alpha == 0)							\ 	  {											\ 	    for (b = 0; b< alpha; b++)								\ 	      dest[b] = src1 [b];								\ 	  }											\ 	else if (src2_alpha == new_alpha){							\ 	  for (b = 0; b< alpha; b++)								\ 	    dest [b] = affect [b] ? src2 [b] : src1 [b];					\ 	} else {										\ 	  ratio = (float) src2_alpha / new_alpha;						\ 	  compl_ratio = 1.0 - ratio;								\ 	  											\ 	  for (b = 0; b< alpha; b++)								\ 	    dest[b] = affect[b] ?								\ 	      (unsigned char) (src2[b] * ratio + src1[b] * compl_ratio + EPSILON) : src1[b];	\ 	}
end_define
...
condition|)
block|{
comment|/*		  printf(" @%p/%p@ ", 			 tile_find_nonmirroring (src_tile), 			 tile_find_nonmirroring (dest_tile));*/
name|tile_mirror
argument_list|(
name|dest_tile
argument_list|,
...
parameter_list|)
block|{
comment|/*   blame all bugs in this function on jaycox@earthlink.net */
specifier|register
name|gint32
name|i
decl_stmt|,
