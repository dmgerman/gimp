...
name|NULL
decl_stmt|;
comment|/* Current line in sliding window    */
name|gint
name|dst_width
decl_stmt|,
name|dst_height
...
name|fv
decl_stmt|;
comment|/* Pos in source image (fract.  part)*/
name|gint
name|u
decl_stmt|,
name|v
...
argument_list|()
expr_stmt|;
comment|/* allocate buffer for 2 * LANCZOS_WIDTH2 + 1 temp source rows      LANCZOS_WIDTH2   : lines are sliding window buffer   */
name|src_rowstride
operator|=
name|src_width
operator|*
...
control|)
block|{
comment|/* 2nd row in src buffer becomes first */
name|rotate_pointers
argument_list|(
name|src
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* Make sure we do not read past the end of image */
name|srcrow
operator|=
operator|(
name|i
...
literal|1
expr_stmt|;
comment|/* append row at the end src buffer*/
name|pos
operator|=
name|LANCZOS_WIDTH2
operator|-
...
expr_stmt|;
block|}
comment|/* current src(v) line in sliding window */
name|cur_buf
operator|=
name|src
index|[
