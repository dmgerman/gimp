...
typedef|typedef
struct|struct
DECL|struct|__anon29908e4e0108
block|{
DECL|member|l
name|gfloat
name|l
...
name|height
decl_stmt|;
DECL|member|pixtoclassresult
name|GHashTable
modifier|*
name|pixtoclassresult
decl_stmt|;
DECL|member|bgsig
name|lab
modifier|*
...
typedef|typedef
struct|struct
DECL|struct|__anon29908e4e0208
block|{
DECL|member|bgdist
name|gfloat
name|bgdist
...
specifier|static
name|void
DECL|function|stageone (lab * points,gint left,gint right,gint depth,gint * clusters,const gfloat * limits,gint dims)
name|stageone
parameter_list|(
name|lab
modifier|*
...
specifier|static
name|void
DECL|function|stagetwo (lab * points,gint left,gint right,gint depth,GSList ** clusters,const gfloat * limits,gfloat threshold,gint dims)
name|stagetwo
parameter_list|(
name|lab
modifier|*
...
name|lab
modifier|*
DECL|function|create_signature (lab * input,gint length,gint * returnlength,const gfloat * limits,gint dims)
...
begin_comment
comment|/* This method checks out the neighbourhood of the pixel at position  * (pos_x,pos_y) in the TileManager mask, it adds the sourrounding  * pixels to the queue to allow further processing it uses maskVal to  * determine if the sourounding pixels have already been visited x,y  * are passed from above.  */
end_comment
...
specifier|static
name|void
DECL|function|find_max_blob (TileManager * mask,gint x,gint y,gint width,gint height,gint sizeFactorToKeep)
name|find_max_blob
parameter_list|(
name|TileManager
modifier|*
...
begin_comment
comment|/**  * siox_init:  * @pixels:      the tiles to extract the foreground from  * @colormap:    colormap in case @pixels are indexed, %NULL otherwise  * @offset_x:    horizontal offset of @pixels with respect to the @mask  * @offset_y:    vertical offset of @pixels with respect to the @mask  * @mask:        a mask indicating sure foreground (255), sure background (0)  *               and undecided regions ([1..254]).  * @x:           horizontal offset into the mask  * @y:           vertical offset into the mask  * @width:       width of working area on mask  * @height:      height of working area on mask  *  * Initializes the SIOX segmentator.  * Creates and returns a SioxState struct that has to be passed to all  * function calls of this module as it maintaines the state.  */
end_comment

begin_function
name|SioxState
modifier|*
DECL|function|siox_init (TileManager * pixels,const guchar * colormap,gint offset_x,gint offset_y,TileManager * mask,gint x,gint y,gint width,gint height,SioxProgressFunc progress_callback,gpointer progress_data)
name|siox_init
parameter_list|(
name|TileManager
modifier|*
...
name|g_printerr
argument_list|(
literal|"siox.c: called siox_init() with bpp=%d, x=%d, y=%d, width=%d, height=%d, offset_x=%d, offset_y=%d\n"
argument_list|,
name|bpp
argument_list|,
name|x
...
begin_comment
comment|/**  * siox_foreground_extract:  * @state:       current state struct as constructed by siox_init  * @sensitivity:    a double array with three entries specifing the accuracy,  *                  a good value is: { 0.64, 1.28, 2.56 }  * @smoothness:     boundary smoothness (a good value is 3)  * @multiblob:      allow multiple blobs (true) or only one (false)  * @refinementtype: bitmask encoding the changes that have been made by the user since last call  *  * Writes the resulting segmentation into @mask.  */
end_comment

begin_function
name|void
DECL|function|siox_foreground_extract (SioxState * state,gint smoothness,const gdouble sensitivity[3],gboolean multiblob,gint refinementtype)
...
name|g_printerr
argument_list|(
literal|"siox.c: called siox_foreground_extract() with refinementtype=%d\n"
argument_list|,
name|refinementtype
argument_list|)
expr_stmt|;
...
begin_function
name|void
DECL|function|siox_drb (SioxState * state,gint x,gint y,gint brushradius,gint brushmode,gfloat threshold)
name|siox_drb
parameter_list|(
name|SioxState
modifier|*
...
name|g_printerr
argument_list|(
literal|"siox.c: siox_done() finished.\n"
argument_list|)
expr_stmt|;
endif|#
directive|endif
