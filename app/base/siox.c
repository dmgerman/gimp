...
typedef|typedef
struct|struct
DECL|struct|__anon291fa9200108
block|{
DECL|member|l
name|gfloat
name|l
...
specifier|static
name|void
DECL|function|threshold_mask (TileManager * mask,guchar maskval,guchar newval,gint x,gint y,gint width,gint height)
name|threshold_mask
parameter_list|(
name|TileManager
modifier|*
...
specifier|static
name|void
DECL|function|set_mask (TileManager * mask,guchar maskval,gint x,gint y,gint width,gint height)
name|set_mask
parameter_list|(
name|TileManager
modifier|*
...
specifier|static
name|void
DECL|function|process_neighbours (gint pos_x,gint pos_y,gint x,gint y,gint width,gint height,GQueue * q,TileManager * mask,guchar maskval)
name|process_neighbours
parameter_list|(
name|gint
name|pos_x
parameter_list|,
...
begin_comment
comment|/* This method processes every position in the queue, it finishes when  * the queeue is empty and no further pixels kann be visited.  */
end_comment

begin_function
specifier|static
name|int
DECL|function|process_queue (GQueue * q,gint x,gint y,gint width,gint height,TileManager * mask,guchar maskval)
name|process_queue
parameter_list|(
name|GQueue
modifier|*
...
name|maskval
expr_stmt|;
comment|/* pixel is set in original selection */
if|if
condition|(
name|val
operator|&
...
begin_comment
comment|/*  * This method finds the biggest connected component in mask, it  * clears everything in mask except the biggest component Pixels that  * should be considererd set in incoming mask, must fullfil (pixel&  * 0x1) the method uses no further memory, except a queue, it finds  * the biggest component by a 2 phase algorithm 1. in the first phase  * the koordinates of an element of the biggest component are  * identified, during set phase all pixels are visited 2. in the  * second phase first visitation flags are reseted, and afterwards a  * connected component starting at the found coordinates is  * determined, this is the biggest component, the result is written  * into mask, all pixels that belong to the biggest component, are set  * to 255 any other to 0.  */
end_comment
...
name|val
decl_stmt|;
comment|/* this mask is used to check if a pixel has been visited */
specifier|const
name|guchar
name|visited
init|=
literal|0x80
decl_stmt|;
comment|/* this mask is used to set a pixel to unvisited          */
specifier|const
name|guchar
name|unvisited
init|=
...
argument_list|)
expr_stmt|;
comment|/* mark every pixel in the mask as unvisited */
name|set_mask
argument_list|(
name|mask
argument_list|,
...
condition|)
continue|continue;
comment|/* mark current pixel as visited */
name|val
operator||=
name|visited
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* if this pixel is not marked as selection in original,                * image skip it                */
if|if
condition|(
operator|!
operator|(
...
condition|)
continue|continue;
comment|/* check out neighbourhood*/
name|process_neighbours
argument_list|(
name|pos_x
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* remember bigest regions size and coords of an element*/
if|if
condition|(
name|regioncount
operator|>
...
block|}
block|}
comment|/* clear visited flag for all pixels */
name|set_mask
argument_list|(
name|mask
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* now push maxblob coords onto stack and find maxblob again, so    * that it can be set as resulting mask    */
comment|/* mark startpixel als visited */
name|read_pixel_data_1
argument_list|(
name|mask
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* set found pixel to 255 in the mask */
name|threshold_mask
argument_list|(
name|mask
argument_list|,
