...
name|printf
argument_list|(
literal|"Hmm somthing was FALSE\n"
argument_list|)
expr_stmt|;
block|}
block|}
...
begin_comment
comment|/* static inline MaskBuf *  blur_mask(MaskBuf *mask) {   MaskBuf *retur;   int y, x;   unsigned char *source;   unsigned char *dest;   int height, width;      retur = mask_buf_new(mask->width, mask->height);    source = mask->data;   dest = retur->data;      width = mask->width;   height = mask->height;    for (y = 1; y< (height -1); y++)     {       for(x = 1; x< (width -1); x++) 	{ 	 	  dest[(y * retur->width + x)] = ( 					  2 * source[((y-1) * width) + x - 1] + 					  2 * source[((y-1) * width) + x] + 					  2 * source[((y-1) * width) + x + 1] + 					  2 * source[(y * width) + x - 1] + 					  source[(y * width) + x] + 					  2 * source[(y * width) + x + 1] + 					  2 * source[((y+1) * width) + x - 1] + 					  2 * source[((y+1) * width) + x] + 					  source[((y+1) * width) + x + 1])/16; 	}     }      mask_buf_free(mask);   return retur; } */
end_comment

begin_comment
comment|/* static inline MaskBuf *  blur_mask(MaskBuf *mask) {   MaskBuf *retur;   int y, x;   unsigned char *source;   unsigned char *dest;   int height, width;      retur = mask_buf_new(mask->width, mask->height);    source = mask->data;   dest = retur->data;      width = mask->width;   height = mask->height;    for (y = 2; y< (height -2); y++)     {       for(x = 2; x< (width -2); x++) 	{ 	 	  dest[(y * retur->width + x)] = (  					    					  2 * source[((y-2) * width) + x - 2] + 					  2 * source[((y-2) * width) + x - 1] + 					  2 * source[((y-2) * width) + x] + 					  2 * source[((y-2) * width) + x + 1] + 					  2 * source[((y-2) * width) + x + 2] + 					    					  2 * source[((y-1) * width) + x - 2] + 					  4 * source[((y-1) * width) + x - 1] + 					  4 * source[((y-1) * width) + x] + 					  4 * source[((y-1) * width) + x + 1] + 					  2 * source[((y-1) * width) + x + 2] +  					  2 * source[(y * width) + x - 2] + 					  4 * source[(y * width) + x - 1] + 					  1 * source[(y * width) + x] + 					  4 * source[(y * width) + x + 1] + 					  1 * source[(y * width) + x + 2] +   					  2 * source[((y+1) * width) + x - 2] + 					  4 * source[((y+1) * width) + x - 1] + 					  4 * source[((y+1) * width) + x] + 					  4 * source[((y+1) * width) + x + 1] + 					  2 * source[((y+1) * width) + x + 2] +  					  2 * source[((y+2) * width) + x - 2] + 					  2 * source[((y+2) * width) + x - 1] + 					  2 * source[((y+2) * width) + x] + 					  2 * source[((y+2) * width) + x + 1] + 					  2 * source[((y+2) * width) + x + 2] 					   					  )/64;  	}     }      mask_buf_free(mask);   return retur;   }    */
end_comment
...
name|MaskBuf
modifier|*
DECL|function|blur_mask (MaskBuf * mask)
name|blur_mask
parameter_list|(
name|MaskBuf
modifier|*
...
condition|)
return|return;
comment|/*print_mask(brush_mask_in);*/
name|brush_mask1
operator|=
name|mask_buf_new
argument_list|(
...
name|brush_mask1
expr_stmt|;
comment|/*mask_buf_free(brush_mask2);     mask_buf_free(brush_mask3);*/
comment|/*print_mask(brush_mask);*/
