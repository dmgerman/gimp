...
block|for (list = vectors->strokes; list; list = g_list_next (list))     {       GimpStroke *stroke = list->data;       GList      *list2;        for (list2 = stroke->anchors; list2; list2 = g_list_next (list2))         {           GimpAnchor *anchor = list2->data;            switch (flip_type)             {             case GIMP_ORIENTATION_HORIZONTAL:               anchor->position.x = -(anchor->position.x - axis) + axis;               break;              case GIMP_ORIENTATION_VERTICAL:               anchor->position.y = -(anchor->position.y - axis) + axis;               break;              default:               break;             }         }     }
endif|#
directive|endif
name|gimp_vectors_thaw
argument_list|(
...
name|GimpVectors
modifier|*
DECL|function|gimp_vectors_convert (GimpVectors * vectors,GimpImage * dest_image)
