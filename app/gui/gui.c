...
begin_comment
comment|/*  * gui_recover:  * @n_recoveries: number of recovered files.  *  * Query the user interactively if files were saved from a previous  * crash, asking whether to try and recover or discard them.  *  * Returns: TRUE if answer is to try and recover, FALSE otherwise.  */
end_comment

begin_function
name|gboolean
DECL|function|gui_recover (gint n_recoveries)
name|gui_recover
parameter_list|(
name|gint
name|n_recoveries
...
name|_
argument_list|(
literal|"Eeek! It looks like GIMP recovered from a crash!"
argument_list|)
argument_list|)
expr_stmt|;
name|gimp_message_box_set_text
...
literal|"Do you want to try and recover them?"
argument_list|,
name|n_recoveries
argument_list|)
argument_list|,
...
name|GdkMonitor
modifier|*
DECL|function|gui_get_initial_monitor (Gimp * gimp)
...
argument_list|()
expr_stmt|;
comment|/* If any modifier is set when quitting (typically when exiting with    * Ctrl-q for instance!), when serializing the tool options, it will    * save any alternate value instead of the main one. Make sure that    * any modifier is reset before saving options.    */
name|active_tool
operator|=
...
specifier|static
name|gboolean
DECL|function|gui_check_action_exists (const gchar * accel_path)
name|gui_check_action_exists
parameter_list|(
specifier|const
name|gchar
