...
block|{
block|{
literal|"/File/Test Dialogs/Multi List..."
block|,
name|NULL
block|,
...
block|{
block|{
literal|"/File/Test Dialogs/Multi Grid..."
block|,
name|NULL
block|,
...
begin_decl_stmt
DECL|variable|n_dialogs_entries
specifier|static
name|guint
name|n_dialogs_entries
init|=
operator|(
sizeof|sizeof
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/*  do nothing if we've already got the filename on the list  */
for|for
control|(
name|list
operator|=
...
condition|)
block|{
comment|/* the following lines would move the entry to the top            * 	   * last_opened_raw_filenames = g_slist_remove_link (last_opened_raw_filenames, list); 	   * last_opened_raw_filenames = g_slist_concat (list, last_opened_raw_filenames); 	   * menus_last_opened_update_labels (); 	   */
return|return;
block|}
block|}
name|num_entries
...
specifier|static
name|void
DECL|function|menus_create_item (GtkItemFactory * item_factory,GimpItemFactoryEntry * entry,gpointer callback_data,guint callback_type,gboolean create_tearoff)
name|menus_create_item
parameter_list|(
name|GtkItemFactory
modifier|*
...
specifier|static
name|void
DECL|function|menus_create_items (GtkItemFactory * item_factory,guint n_entries,GimpItemFactoryEntry * entries,gpointer callback_data,guint callback_type,gboolean create_tearoff)
name|menus_create_items
parameter_list|(
name|GtkItemFactory
modifier|*
...
directive|if
literal|0
block|menu_item = gtk_item_factory_get_widget (image_factory, 					   tool_info[POSTERIZE].menu_path);   if (menu_item&& menu_item->parent)     gtk_menu_reorder_child (GTK_MENU (menu_item->parent), menu_item, 3);    {     static ToolType color_tools[] = { COLOR_BALANCE, 				      HUE_SATURATION, 				      BRIGHTNESS_CONTRAST, 				      THRESHOLD, 				      LEVELS, 				      CURVES };     static gint n_color_tools = (sizeof (color_tools) / 				 sizeof (color_tools[0]));     GtkWidget *separator;     gint i, pos;      pos = 1;      for (i = 0; i< n_color_tools; i++)       { 	menu_item = 	  gtk_item_factory_get_widget (image_factory, 				       tool_info[color_tools[i]].menu_path); 	if (menu_item&& menu_item->parent) 	  { 	    gtk_menu_reorder_child (GTK_MENU (menu_item->parent), 				    menu_item, pos); 	    pos++; 	  }       }     if (menu_item&& menu_item->parent)       { 	separator = gtk_menu_item_new (); 	gtk_menu_insert (GTK_MENU (menu_item->parent), separator, pos); 	gtk_widget_show (separator);       }   }
endif|#
directive|endif
name|filename
operator|=
...
specifier|static
name|void
DECL|function|menus_debug_cmd_callback (GtkWidget * widget,gpointer callback_data,guint callback_action)
name|menus_debug_cmd_callback
parameter_list|(
name|GtkWidget
modifier|*
