...
directive|if
literal|0
block|if (uri)     {       GimpImagefile *documents_imagefile;        documents_imagefile = (GimpImagefile *)         gimp_container_get_child_by_name (imagefile->gimp->documents, uri);        if (GIMP_IS_IMAGEFILE (documents_imagefile)&&           (documents_imagefile != imagefile))         {           gimp_imagefile_update (GIMP_IMAGEFILE (documents_imagefile), size);         }     }
endif|#
directive|endif
block|}
end_function
...
argument_list|)
expr_stmt|;
comment|/*  FIXME: add back .xvpics compatibility  */
if|if
condition|(
name|temp_buf
condition|)
