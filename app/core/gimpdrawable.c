...
begin_comment
comment|/* note:  you may pass a NULL buffer to gimp_drawable_set_buffer() and  * gimp_drawable_set_buffer_full(), in order to clear the drawable's current  * buffer, however, push_undo must be FALSE in that case, and you may only use  * the drawable in a very limited way while it has no buffer.  */
end_comment
