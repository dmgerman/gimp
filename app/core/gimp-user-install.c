...
define|\
value|"\"<Actions>/buffers/buffers-paste-as-new\""  "|" \   "\"<Actions>/edit/edit-paste-as-new\""        "|" \   "\"<Actions>/file/file-export\""              "|" \   "\"<Actions>/file/file-export-to\""           "|" \   "\"<Actions>/layers/layers-text-tool\""       "|" \   "\"<Actions>/plug-in/plug-in-gauss\""         "|" \   "\"<Actions>/tools/tools-value-[1-4]-.*\""    "|" \   "\"<Actions>/vectors/vectors-path-tool\""     "|" \   "\"<Actions>/tools/tools-blend\""
end_define
...
name|match
argument_list|,
literal|"\"<Actions>/tools/tools-blend\""
argument_list|)
operator|==
literal|0
condition|)
...
name|new_value
argument_list|,
literal|"\"<Actions>/tools/tools-gradient\""
argument_list|)
expr_stmt|;
block|}
...
begin_define
DECL|macro|GIMPRESSIONIST_UPDATE_PATTERN
define|#
directive|define
name|GIMPRESSIONIST_UPDATE_PATTERN
define|\
value|"selectedbrush=Brushes/paintbrush.pgm"
end_define

begin_function
specifier|static
name|gboolean
DECL|function|user_update_gimpressionist (const GMatchInfo * matched_value,GString * new_value,gpointer data)
name|user_update_gimpressionist
parameter_list|(
specifier|const
name|GMatchInfo
modifier|*
...
argument_list|)
decl_stmt|;
comment|/* See bug 791934: both brushes are identical. */
if|if
condition|(
name|g_strcmp0
argument_list|(
name|match
argument_list|,
literal|"selectedbrush=Brushes/paintbrush.pgm"
argument_list|)
operator|==
literal|0
condition|)
...
name|new_value
argument_list|,
literal|"selectedbrush=Brushes/paintbrush01.pgm"
argument_list|)
expr_stmt|;
block|}
else|else
...
begin_define
DECL|macro|TOOL_PRESETS_UPDATE_PATTERN
define|#
directive|define
name|TOOL_PRESETS_UPDATE_PATTERN
define|\
value|"GimpImageMapOptions"  "|" \   "GimpBlendOptions"     "|" \   "gimp-blend-tool"      "|" \   "gimp-tool-blend"
end_define

begin_function
specifier|static
name|gboolean
DECL|function|user_update_tool_presets (const GMatchInfo * matched_value,GString * new_value,gpointer data)
name|user_update_tool_presets
parameter_list|(
specifier|const
name|GMatchInfo
modifier|*
...
begin_comment
comment|/* Actually not only for contextrc, but all other files where  * gimp-blend-tool may appear. Apparently that is also "devicerc", as  * well as "toolrc" (but this one is skipped anyway).  */
end_comment

begin_define
DECL|macro|CONTEXTRC_UPDATE_PATTERN
define|#
directive|define
name|CONTEXTRC_UPDATE_PATTERN
value|"gimp-blend-tool"
end_define

begin_function
specifier|static
name|gboolean
DECL|function|user_update_contextrc_over20 (const GMatchInfo * matched_value,GString * new_value,gpointer data)
name|user_update_contextrc_over20
parameter_list|(
specifier|const
name|GMatchInfo
modifier|*
