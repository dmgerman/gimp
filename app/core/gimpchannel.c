...
specifier|static
name|void
DECL|function|gimp_channel_flip (GimpItem * item,GimpOrientationType flip_type,gdouble axis)
name|gimp_channel_flip
parameter_list|(
name|GimpItem
modifier|*
...
name|g_print
argument_list|(
literal|"FIXME: implement channel flipping\n"
argument_list|)
expr_stmt|;
if|#
directive|if
literal|0
block|GimpChannel *channel;   GimpImage   *gimage;   TileManager *tiles;    channel = GIMP_CHANNEL (item);   gimage  = gimp_item_get_image (item);    gimp_image_undo_group_start (gimage, GIMP_UNDO_GROUP_TRANSFORM,                                _("Flip Channel"));    tiles = gimp_drawable_transform_tiles_flip (GIMP_DRAWABLE (channel),                                               GIMP_DRAWABLE (channel)->tiles,                                               flip_type, axis,                                               TRUE
comment|/* always clip_result */
block|);    if (tiles)     gimp_drawable_transform_paste (GIMP_DRAWABLE (channel), tiles, FALSE);    gimp_image_undo_group_end (gimage);
comment|/*  bounds are now unknown  */
block|channel->bounds_known = FALSE;
endif|#
directive|endif
block|}
end_function
