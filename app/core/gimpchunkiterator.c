...
begin_comment
comment|/* the minimal processed area on the current iteration, above which we  * calculate a new target area to render on the next iteration  */
end_comment

begin_define
DECL|macro|MIN_AREA_PER_UPDATE
define|#
directive|define
name|MIN_AREA_PER_UPDATE
value|1024
end_define
...
condition|)
block|{
comment|/* adjust the target area to be processed on the next iteration,        * according to the area processed during this iteration and the elapsed        * time, in order to match the desired interval.        */
if|if
condition|(
name|iter
operator|->
...
name|interval
expr_stmt|;
comment|/* calculate the median target area of the last            * TARGET_AREA_HISTORY_SIZE iterations            */
name|memcpy
argument_list|(
name|target_area_history
argument_list|,
