...
name|_
argument_list|(
literal|"Cannot merge down an invisible layer."
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
argument_list|)
expr_stmt|;
comment|/* if this is a pass-through group, change its mode to NORMAL *before*    * duplicating it, since PASS_THROUGH mode is invalid for regular layers.    * see bug #793714.    */
if|if
condition|(
...
name|composite_mode
decl_stmt|;
comment|/* keep the group's current blend space, composite space, and composite        * mode.        */
name|blend_space
operator|=
...
expr_stmt|;
block|}
comment|/*  Disconnect the bottom-layer node's input  */
name|last_node
operator|=
name|gimp_filter_get_node
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/*  Remove the merged layers from the image  */
for|for
control|(
name|layers
operator|=
