...
name|GimpData
modifier|*
DECL|function|gimp_pattern_native_load (const gchar * filename,gboolean stingy_memory_use,GError ** error)
...
name|domain
operator|==
name|GDK_PIXBUF_ERROR
condition|)
name|g_message
argument_list|(
name|_
argument_list|(
literal|"Filename %s is not a valid GdkPixbuf: %s"
argument_list|)
argument_list|,
name|gimp_filename_to_utf8
argument_list|(
...
return|;
block|}
comment|/*  Read in the pattern name -       First try to get a "comment" option, this could be set for pnm,        jpeg and gif (it isn't yet)       Then try the tEXt option, which is set for pngs,        Then fall back to filename    */
name|utf8_filename
operator|=
...
name|_
argument_list|(
literal|"Problem converting filename to UTF8: %s"
argument_list|)
argument_list|,
operator|(
operator|*
