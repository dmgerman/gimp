...
typedef|typedef
struct|struct
DECL|struct|__anon28823fa90108
block|{
DECL|member|factory
name|GimpDataFactory
modifier|*
...
name|cache
decl_stmt|;
DECL|member|top_directory
specifier|const
name|gchar
modifier|*
name|top_directory
decl_stmt|;
DECL|member|writable_path
name|GList
modifier|*
name|writable_path
decl_stmt|;
...
specifier|static
name|gboolean
DECL|function|gimp_data_factory_is_dir_writable (const gchar * dirname,GList * writable_path)
name|gimp_data_factory_is_dir_writable
parameter_list|(
specifier|const
name|gchar
modifier|*
...
name|FALSE
decl_stmt|;
comment|/*  When processing subdirectories, set the top_directory if it's    *  unset. This way me make sure gimp_data_set_folder_tags()'    *  calling convention is honored: pass NULL when processing the    *  toplevel directory itself, and pass the toplevel directory when    *  processing any folder inside.    */
if|if
condition|(
operator|!
name|context
...
argument_list|)
expr_stmt|;
comment|/*  Unset, the string is only valid within this function, and will    *  be set again for the next subdirectory.    */
if|if
condition|(
name|top_set
condition|)
