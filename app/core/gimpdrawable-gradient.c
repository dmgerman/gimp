...
begin_function
name|void
DECL|function|gimp_drawable_gradient_adjust_coords (GimpDrawable * drawable,GimpGradientType gradient_type,const GeglRectangle * region,gdouble * startx,gdouble * starty,gdouble * endx,gdouble * endy)
name|gimp_drawable_gradient_adjust_coords
parameter_list|(
name|GimpDrawable
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* we potentially adjust the gradient coordinates according to the gradient    * type, so that in cases where the gradient span is not related to the    * segment length, the gradient cache (in GimpOperationGradient) is big    * enough not to produce banding.    */
switch|switch
condition|(
name|gradient_type
condition|)
block|{
comment|/* for conical gradients, use a segment with the original origin and      * direction, whose length is the circumference of the largest circle      * centered at the origin, passing through one of the regions's vertices.      */
case|case
...
block|}
break|break;
comment|/* for shaped gradients, only the segment's length matters; use the      * regions's diagonal, which is the largest possible distance between two      * points in the region.      */
case|case
name|GIMP_GRADIENT_SHAPEBURST_ANGULAR
case|:
case|case
