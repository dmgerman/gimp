...
begin_comment
comment|/* transforms the cubic bezier segment, defined by the four control points  * 'bezier', by 'matrix', performing clipping by the near plane.  *  * returns the transformed set of bezier segments in 't_bezier', and their  * count in 'n_t_bezier'.  the minimal possible number of transformed segments  * is 0, which happens when the entire segment is clipped.  the maximal  * possible number of transformed segments is 2.  *  * if the first transformed segment is an initial segment of 'bezier', sets  * '*start_in' to TRUE, otherwise to FALSE.  if the last transformed segment is  * a final segment of 'bezier', sets '*end_in' to TRUE, otherwise to FALSE.  *  * 't_bezier' may not alias 'bezier'.  */
end_comment

begin_function
name|void
DECL|function|gimp_transform_bezier_coords (const GimpMatrix3 * matrix,const GimpCoords bezier[4],GimpCoords t_bezier[2][4],gint * n_t_bezier,gboolean * start_in,gboolean * end_in)
...
argument_list|)
expr_stmt|;
comment|/* transform the individual control points    *    * TODO:  a perspective-transformed bezier curve is not itself a bezier curve    * in general.  this is therefore only an approximation, in the non-affine    * case.  when the curve is highly nonlinear, we should subdivide it to avoid    * diverging too much from the real transformed curve.    */
for|for
control|(
name|i
operator|=
