...
control|)
block|{
comment|/* We must not use gimp_item_is_position_locked(), especially        * since a child implementation may call the current function and        * end up in infinite loop.        * We are only interested in the value of `lock_position` flag.        */
if|if
condition|(
...
begin_function
name|void
DECL|function|gimp_item_linked_transform (GimpItem * item,GimpContext * context,const GimpMatrix3 * matrix,GimpTransformDirection direction,GimpInterpolationType interpolation_type,GimpTransformResize clip_result,GimpProgress * progress)
