...
condition|)
block|{
comment|/*  This seems to be controversial, see the discussions in issue        *  #3532 and issue #301. Enable the code below to test the        *  proposed syncing of color profile and DCF info.        */
if|#
directive|if
literal|0
block|GimpColorProfile       *profile = gimp_image_get_color_profile (image);       GimpMetadataColorspace  space   = GIMP_METADATA_COLORSPACE_UNSPECIFIED;        if (profile)         {           static GimpColorProfile *adobe = NULL;            if (! adobe)             adobe = gimp_color_profile_new_rgb_adobe ();            if (gimp_color_profile_is_equal (profile, adobe))             space = GIMP_METADATA_COLORSPACE_ADOBERGB;         }       else         {           space = GIMP_METADATA_COLORSPACE_SRGB;         }        gimp_metadata_set_colorspace (metadata, space);
endif|#
directive|endif
block|}
block|}
