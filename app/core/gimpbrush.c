...
directive|if
literal|0
block|if (brush->priv->blured_mask)     {       gimp_temp_buf_unref (brush->priv->blured_mask);       brush->priv->blured_mask = NULL;     }    if (brush->priv->blured_pixmap)     {       gimp_temp_buf_unref (brush->priv->blured_pixmap);       brush->priv->blured_pixmap = NULL;     }    if (brush->priv->mask_cache)     gimp_brush_cache_clear (brush->priv->mask_cache);    if (brush->priv->pixmap_cache)     gimp_brush_cache_clear (brush->priv->pixmap_cache);    if (brush->priv->boundary_cache)     gimp_brush_cache_clear (brush->priv->boundary_cache);
endif|#
directive|endif
block|}
end_function
