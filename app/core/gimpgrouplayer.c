...
name|tiles
decl_stmt|;
comment|/*  FIXME: find a better way to do this: need to set the item's            *  extents to the new values so the projection will create            *  its tiles with the right size            */
name|item
operator|->
name|width
operator|=
...
argument_list|)
expr_stmt|;
comment|/*  FIXME: need to set the item's extents back to the old            *  values so gimp_drawable_set_tiles_full() will emit all            *  signals needed by the layer tree to update itself            */
name|item
operator|->
name|width
operator|=
