...
typedef|typedef
struct|struct
DECL|struct|__anon2ac78bc00108
block|{
DECL|member|buffer
name|GeglBuffer
modifier|*
...
typedef|typedef
struct|struct
DECL|struct|__anon2ac78bc00208
block|{
DECL|member|x
name|gint
name|x
...
specifier|static
name|void
DECL|function|gimp_pickable_contiguous_region_prepare_line_art_async_func (GimpAsync * async,LineArtData * data)
...
name|GeglBuffer
modifier|*
DECL|function|gimp_pickable_contiguous_region_prepare_line_art (GimpPickable * pickable,gboolean select_transparent,gfloat stroke_threshold,gfloat ** distmap,gfloat ** thickmap)
...
condition|)
block|{
comment|/* It is much better experience to pre-compute the line art,            * but it may not be always possible (for instance when            * selecting/filling through a PDB call).            */
name|line_art
operator|=
name|gimp_pickable_contiguous_region_prepare_line_art
argument_list|(
...
condition|)
block|{
comment|/* As a special exception, if you fill over a line art pixel, only        * fill the pixel and exit        */
name|start_col
index|[
literal|0
index|]
...
condition|)
block|{
comment|/* The last step of the line art algorithm is to make sure that        * selections does not leave "holes" between its borders and the        * line arts, while not stepping over as well.        * I used to run the "gegl:watershed-transform" operation to flood        * the stroke pixels, but for such simple need, this simple code        * is so much faster while producing better results.        */
name|gfloat
modifier|*
name|mask
decl_stmt|;
...
name|GIMP_SELECT_CRITERION_LINE_ART
case|:
comment|/* Smart selection is handled before. */
name|g_return_val_if_reached
argument_list|(
literal|0.0
argument_list|)
...
name|GimpPickableLineArtAsyncResult
modifier|*
DECL|function|line_art_result_new (GeglBuffer * line_art,gfloat * distmap,gfloat * thickmap)
name|line_art_result_new
parameter_list|(
name|GeglBuffer
modifier|*
...
specifier|static
name|void
DECL|function|line_art_result_free (GimpPickableLineArtAsyncResult * data)
name|line_art_result_free
parameter_list|(
name|GimpPickableLineArtAsyncResult
modifier|*
