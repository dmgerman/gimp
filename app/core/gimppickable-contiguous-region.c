...
typedef|typedef
struct|struct
DECL|struct|__anon27d8b1da0108
block|{
DECL|member|buffer
name|GeglBuffer
modifier|*
...
condition|)
block|{
comment|/* The last step of the line art algorithm is to make sure that        * selections does not leave "holes" between its borders and the        * line arts, while not stepping over as well.        * To achieve this, I label differently the selection and the rest        * and leave the stroke pixels unlabelled, then I let these        * unknown pixels be labelled by flooding through watershed.        */
name|GeglBufferIterator
modifier|*
name|gi
decl_stmt|;
...
argument_list|()
expr_stmt|;
comment|/* Flag the mask on the line art and create a priority map. */
name|priomap
operator|=
name|gegl_buffer_new
argument_list|(
...
block|}
block|}
comment|/* Watershed the line art. */
name|graph
operator|=
name|gegl_node_new
argument_list|()
...
literal|"operation"
argument_list|,
literal|"gegl:watershed-transform"
argument_list|,
literal|"flag-component"
argument_list|,
literal|0
argument_list|,
literal|"flag"
