...
directive|if
literal|0
block|if (gimp_vectors_get_tattoo (GIMP_VECTORS (vectors)) == tattoo) 	return vectors;
endif|#
directive|endif
block|}
return|return
...
directive|if
literal|0
block|if (GIMP_VECTORS (vectors)->gimage != NULL&&       GIMP_VECTORS (vectors)->gimage != gimage)     {       g_warning ("%s: attempting to add vectors to wrong image.", 		 G_GNUC_PRETTY_FUNCTION);       return FALSE;     }
endif|#
directive|endif
if|if
condition|(
...
block|gimp_vectors_removed (GIMP_VECTORS (vectors));
endif|#
directive|endif
if|if
condition|(
