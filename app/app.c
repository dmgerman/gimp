...
condition|)
block|{
comment|/* Before opening images from command line, check for salvaged images        * and query interactively to know if we should recover or discard        * them.        */
name|GList
modifier|*
...
condition|)
block|{
comment|/* Break ties with the backup directory. */
...
argument_list|)
expr_stmt|;
comment|/* One of the rare exceptions where we should call                    * gimp_image_dirty() directly instead of creating                    * an undo. We want the image to be dirty from                    * scratch, without anything to undo.                    */
...
block|}
block|}
comment|/* Delete backup XCF images. */
for|for
control|(
name|iter
operator|=
