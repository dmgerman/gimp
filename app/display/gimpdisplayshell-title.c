...
name|len
decl_stmt|;
comment|/* This is a ugly hack to prevent this function to be called while        * an image is being converted to indexed (which may happen as        * various context switch happen during progress update). In such        * edge case, we end up in an in-between time where the image is        * seamingly broken as it has no format (hence no profile either).        */
if|if
condition|(
...
condition|)
block|{
comment|/* format string ends within %E-sequence, print literal '%E' */
name|i
operator|+=
name|print
argument_list|(
