...
specifier|static
name|void
DECL|function|gimp_display_shell_center_image_callback (GimpDisplayShell * shell,GtkAllocation * allocation,GtkWidget * canvas)
name|gimp_display_shell_center_image_callback
parameter_list|(
name|GimpDisplayShell
modifier|*
name|shell
...
argument_list|)
expr_stmt|;
comment|/* We only want to center on the axes on which the image is smaller    * than the display canvas. If it is larger, it will be centered on    * that axis later, and if we center on all axis unconditionally, we    * end up with the wrong centering if the image is larger than the    * display canvas.    */
name|center_horizontally
operator|=
name|sw
operator|<
...
argument_list|)
expr_stmt|;
comment|/* Not pretty, but we need to center the image as soon as the canvas    * has got its new size allocated. The centering will be wrong if we    * do it too early, and if we do it too late flickering will occur    * due to the image being rendered twice.    */
name|g_signal_connect_swapped
argument_list|(
name|shell
operator|->
