...
begin_function
name|void
DECL|function|gimp_display_shell_menu_update (GtkItemFactory * item_factory,gpointer data)
...
directive|if
literal|0
block|if (shell)     {       gdisp = shell->gdisp;        if (popup_only)         item_factory = GTK_ITEM_FACTORY (shell->popup_factory);       else         item_factory = GTK_ITEM_FACTORY (shell->menubar_factory);     }   else if (popup_only)     {       item_factory = GTK_ITEM_FACTORY (gimp_item_factory_from_path ("<Image>"));     }
endif|#
directive|endif
if|if
condition|(
...
block|if (! popup_only)     {       GimpContext *user_context;        user_context = gimp_get_user_context (gimp);        if (shell)         {           if (gimp_context_get_display (user_context) == gdisp)             gimp_display_shell_set_menu_sensitivity (shell, gdisp->gimage->gimp,                                                      TRUE);         }       else         {           gimp_display_shell_set_menu_sensitivity (NULL, gdisp->gimage->gimp,                                                    TRUE);         }     }
endif|#
directive|endif
block|}
end_function
