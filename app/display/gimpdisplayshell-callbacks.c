...
directive|if
literal|0
block|FIXME         gimp_item_factory_set_active (GTK_ITEM_FACTORY (shell->menubar_factory), 				      "/View/Fullscreen", fullscreen);          if (shell->gdisp ==             gimp_context_get_display (gimp_get_user_context (gimp))) 	gimp_item_factory_set_active (GTK_ITEM_FACTORY (shell->popup_factory), 				      "/View/Fullscreen", fullscreen);
endif|#
directive|endif
block|}
break|break;
...
name|key
expr_stmt|;
comment|/*  For all modifier keys: call the tools modifier_state *and*                *  oper_update method so tools can choose if they are interested                *  in the release itself or only in the resulting state                */
if|if
condition|(
operator|!
name|gimp_image_is_empty
