...
specifier|static
name|void
DECL|function|gimp_display_shell_dnd_position_item (GimpDisplayShell * shell,GimpImage * image,GimpItem * item)
...
specifier|static
name|void
DECL|function|gimp_display_shell_dnd_fill (GimpDisplayShell * shell,GimpFillOptions * options,const gchar * undo_desc)
...
name|open_as_layers
decl_stmt|;
comment|/* If the app is already being torn down, shell->display might be NULL here.    * Play it safe. */
if|if
condition|(
operator|!
name|shell
...
expr_stmt|;
block|}
comment|/* Something above might have run a few rounds of the main loop. Check        * that shell->display is still there, otherwise ignore this as the app        * is being torn down for quitting. */
if|if
condition|(
name|warn
operator|&&
