...
directive|if
literal|0
comment|/*  disabled because we use gimpdisplayshell-profile now, keep    *  the code around for reference.    */
block|if (config->display_module)     {       GType type = g_type_from_name (config->display_module);        if (g_type_is_a (type, GIMP_TYPE_COLOR_DISPLAY))         {           GimpColorDisplay      *display;           GimpColorDisplayStack *stack;            display = g_object_new (type,                                   "color-config",  config,                                   "color-managed", shell,                                   NULL);            stack = gimp_color_display_stack_new ();            gimp_color_display_stack_add (stack, display);           g_object_unref (display);            return stack;         }     }
endif|#
directive|endif
return|return
name|NULL
