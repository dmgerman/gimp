...
block|if (display == window->active_display)     return;    if (window->active_display)     {       active_shell = GIMP_DISPLAY_SHELL (window->active_display->shell);        g_signal_handlers_disconnect_by_func (active_shell,                                             gimp_image_window_shell_scaled,                                             window);     }    window->active_display = display;
endif|#
directive|endif
name|active_shell
operator|=
