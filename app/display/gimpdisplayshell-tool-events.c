...
include|#
directive|include
file|"tools/gimpsamplepointtool.h"
end_include
...
specifier|static
name|gboolean
name|gimp_display_shell_canvas_tool_events_internal
parameter_list|(
name|GtkWidget
modifier|*
name|canvas
...
specifier|static
name|gboolean
name|gimp_display_shell_check_device
parameter_list|(
name|GimpDisplayShell
modifier|*
name|shell
...
specifier|static
name|void
name|gimp_display_shell_handle_scrolling
parameter_list|(
name|GimpDisplayShell
modifier|*
name|shell
...
specifier|static
name|void
name|gimp_display_shell_released
parameter_list|(
name|GimpDisplayShell
modifier|*
name|shell
...
name|TRUE
return|;
comment|/*  do not process most key events while BUTTON1 is down. We do this          *  so tools keep the modifier state they were in when BUTTON1 was          *  pressed and to prevent accelerators from being invoked.          */
if|if
condition|(
name|kevent
operator|->
...
begin_function
name|void
DECL|function|gimp_display_shell_canvas_grab_notify (GtkWidget * canvas,gboolean was_grabbed,GimpDisplayShell * shell)
...
name|TOOL_EVENTS
argument_list|,
literal|"grab_notify (display %p): was_grabbed = %s"
argument_list|,
name|display
argument_list|,
name|was_grabbed
...
specifier|static
name|gboolean
DECL|function|gimp_display_shell_ruler_button_press (GtkWidget * widget,GdkEventButton * event,GimpDisplayShell * shell,GimpOrientationType orientation)
...
specifier|static
name|gboolean
DECL|function|gimp_display_shell_canvas_tool_events_internal (GtkWidget * canvas,GdkEvent * event,GimpDisplayShell * shell,GdkEvent ** next_event)
name|gimp_display_shell_canvas_tool_events_internal
parameter_list|(
name|GtkWidget
modifier|*
...
name|TRUE
return|;
comment|/*  events on overlays have a different window, but these windows'    *  user_data can still be the canvas, we need to check manually if    *  the event's window and the canvas' window are different.    */
if|if
condition|(
name|event
operator|->
...
argument_list|)
expr_stmt|;
comment|/*  if the event came from a different window than the canvas',        *  check if it came from a canvas child and bail out.        */
if|if
condition|(
...
name|TRUE
return|;
comment|/*  ignore enter notify while we have a grab  */
if|if
condition|(
name|shell
operator|->
...
name|TRUE
return|;
comment|/*  ignore leave notify while we have a grab  */
if|if
condition|(
name|shell
operator|->
...
argument_list|)
expr_stmt|;
comment|/*  ignore focus changes while we have a grab  */
if|if
condition|(
name|shell
operator|->
...
condition|)
block|{
comment|/*  If we don't have a grab, this is a release paired with              *  a button press we intentionally ignored because we had              *  a grab on another device at the time of the press              */
if|if
condition|(
operator|!
name|shell
...
expr_stmt|;
block|}
comment|/* always swallow arrow keys, we don't want focus keynav */
if|if
condition|(
operator|!
name|return_val
...
name|FALSE
expr_stmt|;
comment|/* We need to ungrab the pointer in order to catch                      * button release events.                      */
if|if
condition|(
name|shell
operator|->
...
parameter_list|)
block|{
comment|/* FIXME: need some proper GDK API to figure this */
switch|switch
condition|(
name|key
condition|)
...
specifier|static
name|GdkModifierType
DECL|function|gimp_display_shell_button_to_state (gint button)
name|gimp_display_shell_button_to_state
parameter_list|(
name|gint
name|button
...
specifier|static
name|gboolean
DECL|function|gimp_display_shell_check_device (GimpDisplayShell * shell,GdkEvent * event,gboolean * device_changed)
name|gimp_display_shell_check_device
parameter_list|(
name|GimpDisplayShell
modifier|*
...
condition|)
block|{
comment|/*  While we have a grab, ignore all events from all other devices        *  of the same type        */
if|if
condition|(
name|event
operator|->
...
name|TOOL_EVENTS
argument_list|,
literal|"ignoring key event from '%s' while waiting for event from '%s'\n"
argument_list|,
...
name|TOOL_EVENTS
argument_list|,
literal|"ignoring pointer event from '%s' while waiting for event from '%s'\n"
argument_list|,
name|gdk_device_get_name
argument_list|(
name|device
...
specifier|static
name|void
DECL|function|gimp_display_shell_start_scrolling (GimpDisplayShell * shell,const GdkEvent * event,GdkModifierType state,gint x,gint y)
name|gimp_display_shell_start_scrolling
parameter_list|(
name|GimpDisplayShell
modifier|*
...
specifier|static
name|void
DECL|function|gimp_display_shell_stop_scrolling (GimpDisplayShell * shell,const GdkEvent * event)
name|gimp_display_shell_stop_scrolling
parameter_list|(
name|GimpDisplayShell
modifier|*
...
name|FALSE
expr_stmt|;
comment|/* We may have ungrabbed the pointer when space was released while    * mouse was down, to be able to catch a GDK_BUTTON_RELEASE event.    */
if|if
condition|(
name|shell
operator|->
...
specifier|static
name|void
DECL|function|gimp_display_shell_handle_scrolling (GimpDisplayShell * shell,GdkModifierType state,gint x,gint y)
name|gimp_display_shell_handle_scrolling
parameter_list|(
name|GimpDisplayShell
modifier|*
...
specifier|static
name|void
DECL|function|gimp_display_shell_released (GimpDisplayShell * shell,const GdkEvent * event,const GimpCoords * image_coords)
name|gimp_display_shell_released
parameter_list|(
name|GimpDisplayShell
modifier|*
...
condition|)
block|{
comment|/*  We can't just recreate an operation tool, we must                *  make sure the right stuff gets set on it, so                *  re-activate the procedure that created it instead of                *  just calling gimp_context_tool_changed(). See                *  GimpGeglProcedure and bug #776370.                */
name|GimpImageWindow
modifier|*
name|window
decl_stmt|;
...
name|GdkEvent
modifier|*
DECL|function|gimp_display_shell_compress_motion (GdkEvent * initial_event,GdkEvent ** next_event)
name|gimp_display_shell_compress_motion
parameter_list|(
name|GdkEvent
modifier|*
...
condition|)
block|{
comment|/* Discard previous motion event */
if|if
condition|(
name|last_motion
condition|)
...
block|{
comment|/* Let the caller dispatch the event */
operator|*
name|next_event
operator|=
name|event
