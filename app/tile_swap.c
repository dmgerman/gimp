...
directive|if
literal|0
block|pthread_create (&swapin_thread, NULL,&tile_swap_in_thread, NULL);
endif|#
directive|endif
endif|#
directive|endif
...
directive|if
literal|0
comment|/* disable -- broken */
ifdef|#
directive|ifdef
name|USE_PTHREADS
block|AsyncSwapArgs *args;    args = g_new(AsyncSwapArgs, 1);   args->def_swap_file = def_swap_file;   args->fd = fd;   args->tile = tile;
comment|/* add this tile to the list of tiles for the async swapin task */
block|pthread_mutex_lock (&async_swapin_mutex);   g_slist_append (async_swapin_tiles, args);    if (async_swapin_tiles_end)     async_swapin_tiles_end = async_swapin_tiles_end->next;   else     async_swapin_tiles_end = async_swapin_tiles;      pthread_cond_signal (&async_swapin_signal);   pthread_mutex_unlock (&async_swapin_mutex);
else|#
directive|else
...
begin_endif
unit|static void * tile_swap_in_thread (void *data) {   AsyncSwapArgs *args;   GSList *free_item;    while (1)     {       pthread_mutex_lock (&async_swapin_mutex);        if (!async_swapin_tiles)         {           pthread_cond_wait (&async_swapin_signal,&async_swapin_mutex);         }       args = async_swapin_tiles->data;        free_item = async_swapin_tiles;       async_swapin_tiles = async_swapin_tiles->next;       g_slist_free_1(free_item);       if (!async_swapin_tiles)         async_swapin_tiles_end = NULL;        pthread_mutex_unlock (&async_swapin_mutex);        tile_swap_in_attempt(args->def_swap_file, args->fd, args->tile);        g_free(args);     } }
endif|#
directive|endif
end_endif
