...
condition|)
block|{
comment|/* Case no. 1: Î³> 1                *                * The curve should look like a horizontal                * parabola. Since its curvature is greatest when x is                * small, we add more control points there, so the                * approximation is more accurate. I decided to set the                * length of the consecutive segments to xâ, Î³âxâ, Î³Â²âxâ                * and Î³Â³âxâ and I saw that the curves looked                * good. Still, this is completely arbitrary.                */
name|gdouble
name|dx
init|=
literal|0
...
block|{
comment|/* Case no. 2: Î³< 1                *                * The curve is the same as the one in case no. 1,                * observed through a reflexion along the y = x axis. So                * if we invert Î³ and swap the x and y axes we can use                * the same method as in case no. 1.                */
name|GimpLevelsConfig
modifier|*
name|config_inv
decl_stmt|;
