...
literal|2
argument_list|)
comment|/* In GLib< 2.8.2 on Win32 the argument vector passed to g_spawn*()    * should be in system codepage.    *    * Checking compile-time GLib version is the right thing to do.    * Even if running against GLib>= 2.8.2, code compiled against    * headers from GLib< 2.8.2 use backward-compatible functions that    * still take system codepage. Only code compiled against headers    * from GLib>= 2.8.2 use the g_spawn versions (that actually are    * called g_spawn*_utf8()) that take UTF-8.    */
if|if
condition|(
name|interp
condition|)
...
name|g_error
argument_list|(
literal|"Interpreter %s is a file name with characters not in the system codepage. That doesn't work when GIMP is built against GLib 2.8.1 or earlier."
argument_list|,
name|interp
argument_list|)
expr_stmt|;
...
name|g_error
argument_list|(
literal|"Plug-in %s is a file name with characters not in the system codepage. That doesn't work when GIMP is built against GLib 2.8.1 or earlier."
argument_list|,
name|plug_in
operator|->
name|prog
