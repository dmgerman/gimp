...
specifier|static
name|void
DECL|function|gimp_brush_core_paint_line_pixmap_mask (const Babl * fish,const GimpTempBuf * pixmap_mask,const GimpTempBuf * brush_mask,gfloat * d,gint x,gint y,gint width)
name|gimp_brush_core_paint_line_pixmap_mask
parameter_list|(
specifier|const
name|Babl
...
argument_list|)
expr_stmt|;
comment|/* put the source pixmap's pixels, plus the mask's alpha, into        * one line, so we can use one single call to babl_process() to        * convert the entire line        */
for|for
control|(
name|i
operator|=
...
name|i
decl_stmt|;
comment|/* put the source pixmap's pixels into one line, so we can use        * one single call to babl_process() to convert the entire line        */
for|for
control|(
name|i
operator|=
