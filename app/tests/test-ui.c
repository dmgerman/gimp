...
specifier|static
name|int
name|gimp_ui_aux_data_eqiuvalent
parameter_list|(
name|gconstpointer
name|_a
parameter_list|,
...
directive|if
literal|0
block|Gimp        *gimp      = GIMP (data);   GimpImage   *image     = GIMP_IMAGE (gimp_get_image_iter (gimp)->data);   GimpChannel *selection = gimp_image_get_mask (image);   GimpLayer   *active_layer;   GtkWidget   *dockable;   GtkWidget   *gtk_tree_view;   gint         assumed_layer_x;   gint         assumed_empty_layer_y;   gint         assumed_background_layer_y;
...
block|assumed_layer_x            = 96;   assumed_empty_layer_y      = 16;   assumed_background_layer_y = 42;
comment|/* Store the active layer, it shall not change during the execution    * of this test    */
block|active_layer = gimp_image_get_active_layer (image);
...
specifier|static
name|void
DECL|function|maximize_state_in_aux_data (gconstpointer data)
...
expr_stmt|;
block|}
comment|/* Set the aux info to out target data */
name|aux_info
operator|=
name|g_list_append
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Give the WM a chance to maximize/unmaximize us */
name|gimp_test_run_mainloop_until_idle
argument_list|()
expr_stmt|;
name|g_usleep
...
argument_list|()
expr_stmt|;
comment|/* Make sure the maximize/unmaximize happened */
name|g_assert
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Make sure we can read out the window state again */
name|aux_info
operator|=
...
literal|4
decl_stmt|;
comment|/* Undo all changes so we don't need to find the 'Do you want to    * save?'-dialog and its 'No' button    */
while|while
condition|(
name|undo_count
operator|--
...
argument_list|()
expr_stmt|;
comment|/* Did it really disappear? */
name|g_assert_cmpint
argument_list|(
name|g_list_length
argument_list|(
...
name|monitor
operator|=
name|gdk_display_get_primary_monitor
argument_list|(
...
specifier|static
name|int
DECL|function|gimp_ui_aux_data_eqiuvalent (gconstpointer _a,gconstpointer _b)
name|gimp_ui_aux_data_eqiuvalent
parameter_list|(
name|gconstpointer
name|_a
...
specifier|static
name|void
DECL|function|gimp_ui_switch_window_mode (Gimp * gimp)
name|gimp_ui_switch_window_mode
parameter_list|(
name|Gimp
modifier|*
