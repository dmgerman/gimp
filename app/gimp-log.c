...
condition|)
block|{
comment|/*  g_parse_debug_string() has special treatment of the string 'help',        *  but we want to use it for the GIMP_LOG_HELP domain. "list-all"        *  is a replacement for "help" in GIMP.        */
if|if
condition|(
...
condition|)
block|{
comment|/* If the environment variable was set but no log flags are            * set as a result, let's assume one is not sure how to use            * the log flags and output the list of keys as a helper.            */
name|gimp_log_flags
operator|=
name|g_parse_debug_string
argument_list|(
