...
name|gimp_operation_mask_components_prepare
expr_stmt|;
comment|//operation_class->process = gimp_operation_mask_components_parent_process;
name|point_class
operator|->
name|process
operator|=
...
directive|if
literal|0
block|if (self->mask == 0)     {       GObject *input = gegl_operation_context_get_object (context, "input");        gegl_operation_context_set_object (context, "output", input);        return TRUE;     }   else if (self->mask == GIMP_COMPONENT_MASK_ALL)     {       GObject *aux = gegl_operation_context_get_object (context, "aux");        gegl_operation_context_set_object (context, "output", aux);        return TRUE;     }
endif|#
directive|endif
return|return
name|GEGL_OPERATION_CLASS
...
directive|if
literal|0
block|if (self->mask == 0)   {     if (in_buf != out_buf)       memcpy (out_buf, in_buf, sizeof (float) * 4 * samples);   }   else if (self->mask == GIMP_COMPONENT_MASK_ALL&& aux_buf)   {     if (aux_buf != out_buf)       memcpy (out_buf, aux_buf, sizeof (float) * 4 * samples);   }   else
endif|#
directive|endif
block|{
if|if
