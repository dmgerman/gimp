...
directive|if
literal|0
comment|/*  this code tries to be smart but is in fact just a too stupid    *  copy from gegl's normal mode. to fix it, it needs to take    *  mask and opacity into account    */
block|const GeglRectangle *in_extent  = NULL;   const GeglRectangle *aux_extent = NULL;   GObject             *input;   GObject             *aux;
comment|/* get the raw values this does not increase the reference count */
block|input = gegl_operation_context_get_object (context, "input");   aux   = gegl_operation_context_get_object (context, "aux");
comment|/* pass the input/aux buffers directly through if they are not    * overlapping    */
block|if (input)     in_extent = gegl_buffer_get_abyss (GEGL_BUFFER (input));    if (! input ||       (aux&& ! gegl_rectangle_intersect (NULL, in_extent, result)))     {       gegl_operation_context_set_object (context, "output", aux);       return TRUE;     }    if (aux)     aux_extent = gegl_buffer_get_abyss (GEGL_BUFFER (aux));    if (! aux ||       (input&& ! gegl_rectangle_intersect (NULL, aux_extent, result)))     {       gegl_operation_context_set_object (context, "output", input);       return TRUE;     }
endif|#
directive|endif
