...
argument_list|)
decl_stmt|;
comment|/* if we're not the last node, or we're using the opacity/mask shortcut    * function, forward directly to the real process function.    */
if|if
condition|(
operator|!
name|point
...
return|;
block|}
comment|/* otherwise, switch the composite mode temporarily to src-over, before    * handing processing over to the real process function.    */
else|else
block|{
...
specifier|static
name|gboolean
DECL|function|process_layer_only (GeglOperation * operation,void * in_buf,void * layer_buf,void * mask_buf,void * out_buf,glong samples,const GeglRectangle * roi,gint level)
name|process_layer_only
parameter_list|(
name|GeglOperation
modifier|*
