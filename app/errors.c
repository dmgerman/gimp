...
return|;
block|}
comment|/* This is a trick to get the stack trace inside a string.    * GLib's g_on_error_stack_trace() unfortunately writes directly to    * the standard output, which is a very unfortunate implementation.    */
name|pid
operator|=
name|fork
argument_list|()
...
comment|/* XXX I just don't understand why, but somehow the parent process        * doesn't get the output if I don't print something first. I just        * leave this very dirty hack until I figure out what's going on.        */
name|printf
argument_list|(
literal|" "
argument_list|)
...
condition|)
block|{
comment|/* Empty strings are the same as no strings. */
name|g_free
argument_list|(
name|trace
argument_list|)
