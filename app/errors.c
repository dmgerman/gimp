...
directive|ifdef
name|HAVE_EXCHNDL
comment|/* On Win32, the trace has already been processed by ExcHnl            * and is waiting for us in a text file.            * We just want to spawn the trace GUI and exit GIMP directly.            */
if|if
condition|(
name|g_file_test
argument_list|(
...
directive|else
comment|/* On Unix machines, the spawned process will attach to the            * main GIMP process and will generate the backtrace. So we            * run it as a synced process.            */
if|if
condition|(
name|g_spawn_sync
argument_list|(
...
begin_comment
comment|/* In some error cases (e.g. segmentation fault), trying to allocate  * more memory will trigger more segmentation faults and therefore loop  * our error handling (which is just wrong). Therefore printing to a  * file description is an implementation without any memory allocation.  * On the other hand, if trace is not #NULL, a newly-allocated string  * will be returned.  */
end_comment

begin_function
specifier|static
name|gboolean
DECL|function|gimp_print_stack_trace (FILE * file,gchar ** trace)
name|gimp_print_stack_trace
parameter_list|(
name|FILE
modifier|*
...
begin_comment
comment|/* This is mostly the same as g_on_error_query() except that we use our  * own backtrace function, much more complete.  */
end_comment

begin_function
specifier|static
name|void
DECL|function|gimp_on_error_query (void)
name|gimp_on_error_query
parameter_list|(
name|void
parameter_list|)
