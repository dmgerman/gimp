...
begin_comment
comment|/* static void crop_selection_callback (GtkWidget *, gpointer); */
end_comment
...
begin_comment
comment|/* static void crop_selection_callback (GtkWidget *w, 			 gpointer   client_data) {   Tool * tool;   Crop * crop;   GDisplay * gdisp;    tool = active_tool;   crop = (Crop *) tool->private;   gdisp = (GDisplay *) tool->gdisp_ptr;    draw_core_pause (crop->core, tool);   if (! gimage_mask_bounds (gdisp->gimage,&crop->tx1,&crop->ty1,&crop->tx2,&crop->ty2))     {       crop->tx1 = crop->ty1 = 0;       crop->tx2 = gdisp->gimage->width;       crop->ty2 = gdisp->gimage->height;     }    crop_recalc (tool, crop);   draw_core_resume (crop->core, tool); } */
end_comment
