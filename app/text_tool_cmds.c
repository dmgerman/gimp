...
name|gchar
modifier|*
DECL|function|text_xlfd_insert_size (gchar * fontname,gdouble size,SizeType metric,gdouble xresolution,gdouble yresolution,gboolean antialias)
...
name|SUPERSAMPLE
expr_stmt|;
comment|/*  Note: if metric == POINTS we either use the passed resolution (if    *        statement below) or we (implicitly) use the resolution hidden    *        in the fontname.    */
if|if
condition|(
name|metric
operator|==
...
condition|)
block|{
comment|/*  default to pixels by passing 0.0 as resolution  */
name|real_fontname
operator|=
name|text_xlfd_insert_size
argument_list|(
...
literal|"border is specified as -1, empty spaces around the text will not be cropped."
block|,
...
literal|"The border is measured in pixels. If the border is specified as -1, empty spaces around the text will not be cropped."
block|,
