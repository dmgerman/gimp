...
begin_comment
comment|/* static void file_prefs_float_spinbutton_callback (GtkWidget *, gpointer); */
end_comment
...
begin_comment
comment|/* static   int          last_type = RGB; ??? */
end_comment
...
begin_decl_stmt
DECL|variable|default_size_sizeentry
specifier|static
name|GtkWidget
modifier|*
...
begin_decl_stmt
DECL|variable|default_resolution_sizeentry
specifier|static
name|GtkWidget
modifier|*
...
begin_decl_stmt
DECL|variable|monitor_resolution_sizeentry
specifier|static
name|GtkWidget
modifier|*
...
begin_comment
comment|/* Some information regarding preferences, compiled by Raph Levien 11/3/97.    updated by Michael Natterer 27/3/99     The following preference items cannot be set on the fly (at least    according to the existing pref code - it may be that changing them    so they're set on the fly is not hard).     temp-path    swap-path    brush-path    pattern-path    plug-in-path    module-path    palette-path    gradient-path    stingy-memory-use    tile-cache-size    install-cmap    cycled-marching-ants    last-opened-size     All of these now have variables of the form edit_temp_path, which    are copied from the actual variables (e.g. temp_path) the first time    the dialog box is started.     Variables of the form old_temp_path represent the values at the    time the dialog is opened - a cancel copies them back from old to    the real variables or the edit variables, depending on whether they    can be set on the fly.     Here are the remaining issues as I see them:     Still no settings for default-brush, default-gradient,    default-palette, default-pattern, gamma-correction, color-cube.    No widget for confirm-on-close although a lot of stuff is there.     The semantics of "save" are a little funny - it only saves the    settings that are different from the way they were when the dialog    was opened. So you can set something, close the window, open it    up again, click "save" and have nothing happen. To change this    to more intuitive semantics, we should have a whole set of init_    variables that are set the first time the dialog is opened (along    with the edit_ variables that are currently set). Then, the save    callback checks against the init_ variable rather than the old_. */
end_comment
...
begin_comment
comment|/*  commented out because it's not used  static void file_prefs_float_spinbutton_callback (GtkWidget *widget, 				      gpointer   data) {   float *val;    val = data;   *val = gtk_spin_button_get_value_as_float (GTK_SPIN_BUTTON (widget)); } */
end_comment
...
begin_comment
comment|/*  local callback of gimp_dialog_new ()  */
end_comment
...
name|GtkWidget
modifier|*
DECL|function|gimp_dialog_new (const gchar * title,const gchar * wmclass_name,gchar * label1,GtkSignalFunc callback1,gpointer data1,gboolean default_action1,gboolean connect_delete1,...)
...
name|wmclass_name
parameter_list|,
comment|/* this is an action_area button */
name|gchar
modifier|*
name|label1
parameter_list|,
...
parameter_list|,
name|gboolean
name|default_action1
parameter_list|,
name|gboolean
name|connect_delete1
parameter_list|,
comment|/* more action_area buttons */
modifier|...
parameter_list|)
block|{
name|GtkWidget
...
name|GtkWidget
modifier|*
DECL|function|gimp_option_menu_new (GtkSignalFunc menu_item_callback,gpointer initial,gchar * label1,gpointer data1,gpointer set_data1,...)
...
name|GtkWidget
modifier|*
DECL|function|gimp_radio_group_new (GtkSignalFunc radio_button_callback,gpointer initial,gchar * label1,gpointer data1,gpointer set_data1,...)
...
name|GtkWidget
modifier|*
DECL|function|gimp_spin_button_new (gfloat value,gfloat lower,gfloat upper,gfloat step_increment,gfloat page_increment,gfloat page_size,gfloat climb_rate,guint digits)
...
begin_comment
comment|/*  add correctly aligned label& widget to a prefs table  */
end_comment

begin_function
specifier|static
name|void
DECL|function|file_pref_table_attach (GtkTable * table,GtkLabel * label,GtkWidget * widget,gint row)
name|file_pref_table_attach
parameter_list|(
name|GtkTable
modifier|*
name|table
...
expr_stmt|;
block|{
DECL|struct|__anon275a75000108
specifier|static
specifier|const
struct|struct
block|{
...
block|}
block|{
DECL|struct|__anon275a75000208
specifier|static
specifier|const
struct|struct
block|{
