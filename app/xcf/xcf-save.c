...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_int32 (info, data, count,&tmp_error);                    \   if (tmp_error)                                                      \     {                                                                 \       g_propagate_error (error, tmp_error);                           \       return FALSE;                                                   \     }                                                                 \   } G_STMT_END
end_define
...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_offset (info, data, count,&tmp_error);                    \   if (tmp_error)                                                       \     {                                                                  \       g_propagate_error (error, tmp_error);                            \       return FALSE;                                                    \     }                                                                  \   } G_STMT_END
end_define
...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_zero_offset (info, count,&tmp_error);                    \   if (tmp_error)                                                      \     {                                                                 \       g_propagate_error (error, tmp_error);                           \       return FALSE;                                                   \     }                                                                 \   } G_STMT_END
end_define
...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_int8 (info, data, count,&tmp_error);                    \   if (tmp_error)                                                     \     {                                                                \       g_propagate_error (error, tmp_error);                          \       return FALSE;                                                  \     }                                                                \   } G_STMT_END
end_define
...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_float (info, data, count,&tmp_error);                    \   if (tmp_error)                                                      \     {                                                                 \       g_propagate_error (error, tmp_error);                           \       return FALSE;                                                   \     }                                                                 \   } G_STMT_END
end_define
...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_string (info, data, count,&tmp_error);                    \   if (tmp_error)                                                       \     {                                                                  \       g_propagate_error (error, tmp_error);                            \       return FALSE;                                                    \     }                                                                  \   } G_STMT_END
end_define

begin_define
DECL|macro|xcf_write_component_check_error (info,bpc,data,count)
define|#
directive|define
name|xcf_write_component_check_error
parameter_list|(
name|info
parameter_list|,
name|bpc
...
name|count
parameter_list|)
value|G_STMT_START { \   xcf_write_component (info, bpc, data, count,&tmp_error);          \   if (tmp_error)                                                     \     {                                                                \       g_propagate_error (error, tmp_error);                          \       return FALSE;                                                  \     }                                                                \   } G_STMT_END
end_define
...
name|cp
expr_stmt|;
comment|/* seek to the layer offset and save the layer */
name|xcf_check_error
argument_list|(
name|xcf_seek_pos
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* the next layer's offset is after the layer we just wrote */
name|offset
operator|=
name|info
operator|->
...
name|cp
expr_stmt|;
comment|/* seek to the channel offset and save the channel */
name|xcf_check_error
argument_list|(
name|xcf_seek_pos
argument_list|(
...
condition|)
block|{
comment|/* save the new property before the old one, so loading can skip        * the latter        */
name|xcf_check_error
argument_list|(
name|xcf_save_prop
argument_list|(
...
condition|)
comment|/* Do not save the identity symmetry. */
continue|continue;
name|parasite
operator|=
...
argument_list|)
expr_stmt|;
comment|/* if blend_space is AUTO, save the negative of the actual value AUTO          * maps to for the given mode, so that we can correctly load it even if          * the mapping changes in the future.          */
if|if
condition|(
name|blend_space
operator|==
...
argument_list|)
expr_stmt|;
comment|/* if composite_space is AUTO, save the negative of the actual value          * AUTO maps to for the given mode, so that we can correctly load it          * even if the mapping changes in the future.          */
if|if
condition|(
name|composite_space
operator|==
...
argument_list|)
expr_stmt|;
comment|/* if composite_mode is AUTO, save the negative of the actual value          * AUTO maps to for the given mode, so that we can correctly load it          * even if the mapping changes in the future.          */
if|if
condition|(
name|composite_mode
operator|==
...
control|)
block|{
comment|/* Do not save custom guides. */
if|if
condition|(
...
specifier|static
name|gboolean
DECL|function|xcf_save_buffer (XcfInfo * info,GeglBuffer * buffer,GError ** error)
name|xcf_save_buffer
parameter_list|(
name|XcfInfo
modifier|*
...
name|cp
expr_stmt|;
comment|/* seek to the level offset and save the level */
name|xcf_check_error
argument_list|(
name|xcf_seek_pos
argument_list|(
...
expr_stmt|;
block|}
comment|/* the next level's offset if after the level we just wrote */
name|offset
operator|=
name|info
operator|->
...
specifier|static
name|gboolean
DECL|function|xcf_save_level (XcfInfo * info,GeglBuffer * buffer,GError ** error)
name|xcf_save_level
parameter_list|(
name|XcfInfo
modifier|*
...
name|n_tile_cols
expr_stmt|;
comment|/* allocate an offset table so we don't have to seek back after each    * tile, see bug #686862. allocate ntiles + 1 slots because a zero    * offset indicates the offset table's end.    */
name|offset_table
operator|=
name|g_alloca
argument_list|(
...
name|offset_table
expr_stmt|;
comment|/* 'saved_pos' is the offset of the tile offset table  */
name|saved_pos
operator|=
name|info
operator|->
...
name|rect
decl_stmt|;
comment|/* store the offset in the table and increment the next pointer */
operator|*
name|next_offset
operator|++
operator|=
...
return|;
block|}
comment|/* the next tile's offset is after the tile we just wrote */
name|offset
operator|=
name|info
operator|->
...
expr_stmt|;
block|}
comment|/* seek back to the offset table and write it  */
name|xcf_check_error
argument_list|(
name|xcf_seek_pos
argument_list|(
...
name|t
decl_stmt|;
comment|/* if we came here because of a new run, backup one */
if|if
condition|(
operator|!
operator|(
...
specifier|static
name|gboolean
DECL|function|xcf_save_tile_zlib (XcfInfo * info,GeglBuffer * buffer,GeglRectangle * tile_rect,const Babl * format,GError ** error)
name|xcf_save_tile_zlib
parameter_list|(
name|XcfInfo
modifier|*
...
argument_list|)
decl_stmt|;
comment|/* The buffer for compressed data. */
name|guchar
modifier|*
name|buf
init|=
...
name|g_printerr
argument_list|(
literal|"xcf: tile compression failed: %s"
argument_list|,
name|zError
argument_list|(
name|status
