...
decl_stmt|;
name|gint
name|n_broken_layers
init|=
literal|0
decl_stmt|;
name|gint
name|n_broken_channels
init|=
literal|0
decl_stmt|;
name|GList
...
name|_
argument_list|(
literal|"Invalid image mode and precision combination."
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
...
expr_stmt|;
block|}
comment|/* check for symmetry parasites */
name|syms
operator|=
...
goto|;
block|}
comment|/* Don't just stop at the first broken layer. Load as much as            * possible.            */
if|if
condition|(
operator|!
name|item_path
...
condition|)
block|{
comment|/* Item paths may be a problem when layers are missing. */
name|xcf_fix_item_path
argument_list|(
name|layer
argument_list|,
...
name|XCF
argument_list|,
literal|"prop sample point x=%d y=%d mode=%d"
argument_list|,
name|x
argument_list|,
name|y
...
name|n_sample_points
decl_stmt|;
comment|/* if there are already sample points, we loaded the new              * prop before              */
if|if
condition|(
...
name|XCF
argument_list|,
literal|"prop old sample point x=%d y=%d"
argument_list|,
name|x
argument_list|,
name|y
...
name|g_printerr
argument_list|(
literal|"unexpected item offset_x (%d) in XCF, "
literal|"setting to 0\n"
argument_list|,
name|offset_x
argument_list|)
expr_stmt|;
...
name|g_printerr
argument_list|(
literal|"unexpected item offset_y (%d) in XCF, "
literal|"setting to 0\n"
argument_list|,
name|offset_y
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* if blend_space< 0 it was originally AUTO, and its negative is              * the actual value AUTO used to map to at the time the file was              * saved.  if AUTO still maps to the same value, keep using AUTO              * for the property; otherwise, use the concrete value.              */
if|if
condition|(
name|blend_space
operator|<
...
argument_list|)
expr_stmt|;
comment|/* if composite_space< 0 it was originally AUTO, and its negative              * is the actual value AUTO used to map to at the time the file was              * saved.  if AUTO still maps to the same value, keep using AUTO              * for the property; otherwise, use the concrete value.              */
if|if
condition|(
name|composite_space
operator|<
...
name|XCF
argument_list|,
literal|"COMPOSITE_SPACE: AUTO => %d"
argument_list|,
name|composite_space
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* if composite_mode< 0 it was originally AUTO, and its negative              * is the actual value AUTO used to map to at the time the file was              * saved.  if AUTO still maps to the same value, keep using AUTO              * for the property; otherwise, use the concrete value.              */
if|if
condition|(
name|composite_mode
operator|<
...
specifier|static
name|gboolean
DECL|function|xcf_check_layer_props (XcfInfo * info,GList ** item_path,gboolean * is_group_layer,gboolean * is_text_layer)
name|xcf_check_layer_props
parameter_list|(
name|XcfInfo
modifier|*
...
name|cp
expr_stmt|;
comment|/* Load item path and check if this is a group or text layer. */
name|xcf_check_layer_props
argument_list|(
name|info
argument_list|,
...
condition|)
block|{
comment|/* Something is wrong, but leave a chance to the layer because            * anyway group and text layer depends on their contents.            */
name|width
operator|=
name|height
operator|=
...
specifier|static
name|gboolean
DECL|function|xcf_item_path_is_parent (GList * path,GList * parent_path)
name|xcf_item_path_is_parent
parameter_list|(
name|GList
modifier|*
...
specifier|static
name|void
DECL|function|xcf_fix_item_path (GimpLayer * layer,GList ** path,GList * broken_paths)
name|xcf_fix_item_path
parameter_list|(
name|GimpLayer
modifier|*
...
condition|)
block|{
comment|/* Not much to do when the absent path is a parent. */
name|g_printerr
argument_list|(
literal|"%s: layer '%s' moved to layer tree root because of missing parent."
argument_list|,
name|G_STRFUNC
argument_list|,
name|gimp_object_get_name
...
block|}
block|}
comment|/* Check if a parent of path, or path itself is on the same    * tree level as any broken path; and if so, and if the broken path is    * in a lower position in the item group, decrement it.    */
for|for
control|(
name|iter
operator|=
...
name|data
condition|)
comment|/* Paths diverged before reaching iter2 leaf. */
break|break;
if|if
condition|(
name|iter2
