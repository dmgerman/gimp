...
begin_comment
comment|/* static void realize_text (GtkWidget *text, gpointer data) {   gtk_text_set_word_wrap   (GTK_TEXT (text), TRUE);   gtk_text_freeze (GTK_TEXT (text));   gtk_text_insert (GTK_TEXT (text), NULL,&text->style->black, NULL, 		   "You are attempting to convert an image with alpha/layers from RGB/GRAY to INDEXED.  You should not generate a palette of more than 255 colors if you intend to create a transparent or animated GIF file from this image.", 		   -1);      gtk_text_thaw (GTK_TEXT (text)); } */
end_comment
...
name|gtk_label_new
argument_list|(
literal|"You are attempting to convert an image with alpha/layers from RGB/GRAY to INDEXED.\n"
literal|"\tYou should not generate a palette of more than 255 colors if you intend to create a transparent or animated GIF file from this image."
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* 	table = gtk_table_new (2, 1, FALSE); 	{ 	  gtk_container_set_border_width (GTK_CONTAINER (table), 1); 	  gtk_container_add (GTK_CONTAINER (frame), table);  	  text = gtk_text_new (NULL, NULL); 	  { 	    gtk_table_attach (GTK_TABLE (table), text, 0, 1, 0, 1, 			      GTK_FILL | GTK_EXPAND, 			      GTK_FILL | GTK_EXPAND | GTK_SHRINK, 0, 0); 	    gtk_signal_connect (GTK_OBJECT (text), "realize", 				GTK_SIGNAL_FUNC (realize_text), NULL); 	  } 	  gtk_widget_show (text);  	  scrollbar = gtk_vscrollbar_new (GTK_TEXT (text)->vadj); 	  { 	    gtk_table_attach (GTK_TABLE (table), scrollbar, 1, 2, 0, 1, 			      GTK_FILL, GTK_EXPAND | GTK_FILL | GTK_SHRINK, 			      0, 0); 	  } 	  gtk_widget_show (scrollbar); 	} 	gtk_widget_show(table); 	*/
block|}
name|gtk_widget_show
argument_list|(
name|frame
