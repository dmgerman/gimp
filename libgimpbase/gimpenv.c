...
block|{
name|gboolean
name|gimp3_directory_changed
init|=
name|FALSE
decl_stmt|;
comment|/* We have constructed the gimp_dir already. We can return        * gimp_dir unless some parameter gimp_dir depends on has        * changed. For now we just check for changes to GIMP3_DIRECTORY        */
name|gimp3_directory_changed
operator|=
operator|(
name|env_gimp_dir
...
block|}
block|}
comment|/* Remember the GIMP3_DIRECTORY to next invocation so we can check    * if it changes    */
name|g_free
argument_list|(
name|last_env_gimp_dir
argument_list|)
...
name|dup
decl_stmt|;
comment|/*  check for duplicate entries, see bug #784502  */
for|for
control|(
name|dup
operator|=
...
break|break;
block|}
comment|/*  only add to the list if it's not a duplicate  */
if|if
condition|(
operator|!
name|dup
