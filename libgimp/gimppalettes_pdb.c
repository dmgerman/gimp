...
begin_comment
comment|/**  * gimp_palettes_get_list:  * @filter: An optional regular expression used to filter the list.  * @num_palettes: (out) The number of palettes in the list.  *  * Retrieves a list of all of the available palettes  *  * This procedure returns a complete listing of available palettes.  * Each name returned can be used as input to the command  * gimp_context_set_palette().  *  * Returns: (element-type gchar*) (transfer full) The list of palette  * names. The returned value must be freed with g_strfreev().  **/
end_comment
