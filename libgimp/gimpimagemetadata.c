...
directive|if
literal|0
block|{         gchar *xml = gimp_metadata_serialize (metadata);
comment|//        FILE *f = fopen ("/tmp/gimp-test-xml1", "w");
block|FILE *f = fopen ("e:\\gimp-test-xml1", "w");         fprintf (f, "%s", xml);         fclose (f);          GimpMetadata *new = gimp_metadata_deserialize (xml);          gimp_metadata_print (new);          gchar *xml2 = gimp_metadata_serialize (new);
comment|//        f = fopen ("/tmp/gimp-test-xml2", "w");
block|f = fopen ("e:\\gimp-test-xml2", "w");         fprintf (f, "%s", xml2);         fclose (f);
comment|//        system ("diff -u /tmp/gimp-test-xml1 /tmp/gimp-test-xml2");
block|g_free (xml);         g_free (xml2);         g_object_unref (new);       }
endif|#
directive|endif
...
begin_function
name|void
DECL|function|gimp_image_metadata_load_finish (gint32 image_ID,gint32 layer_ID,const gchar * mime_type,GimpMetadata * metadata,GimpMetadataLoadFlags flags,gboolean interactive)
...
expr_stmt|;
comment|/* this is a GimpMetadata XML-Packet    * it is deserialized to a GimpMetadata object.    * Deserialization fills the GExiv2Metadata object of the GimpMetadata*/
name|metadata
operator|=
...
condition|)
block|{
comment|/* we need the attribute-value here, because some xmp-tags cannot be read out and stored like exif tags:                * xmp-tags allow structures like lists. These structures need this special handling. */
name|attribute
operator|=
name|gimp_metadata_get_attribute
argument_list|(
...
condition|)
block|{
comment|/* we need the attribute-value here, because iptc-tags of TYPE_MULTIPLE cannot be read out and stored like exif tags:                * These tags need this special handling. */
name|attribute
operator|=
name|gimp_metadata_get_attribute
argument_list|(
