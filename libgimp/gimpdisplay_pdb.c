...
begin_comment
comment|/**  * gimp_display_is_valid:  * @display: The display to check.  *  * Returns TRUE if the display is valid.  *  * This procedure checks if the given display ID is valid and refers to  * an existing display.  *  * Returns: Whether the display ID is valid.  *  * Since: 2.4  **/
end_comment

begin_function
name|gboolean
DECL|function|gimp_display_is_valid (GimpDisplay * display)
...
begin_comment
comment|/**  * _gimp_display_is_valid: (skip)  * @display_ID: The display to check.  *  * Returns TRUE if the display is valid.  *  * This procedure checks if the given display ID is valid and refers to  * an existing display.  *  * Returns: Whether the display ID is valid.  *  * Since: 2.4  **/
end_comment

begin_function
name|gboolean
DECL|function|_gimp_display_is_valid (gint32 display_ID)
...
begin_comment
comment|/**  * _gimp_display_new: (skip)  * @image_ID: The image.  *  * Create a new display for the specified image.  *  * Creates a new display for the specified image. If the image already  * has a display, another is added. Multiple displays are handled  * transparently by GIMP. The newly created display is returned and can  * be subsequently destroyed with a call to gimp_display_delete(). This  * procedure only makes sense for use with the GIMP UI, and will result  * in an execution error if called when GIMP has no UI.  *  * Returns: The new display.  **/
end_comment

begin_function
name|gint32
DECL|function|_gimp_display_new (gint32 image_ID)
...
begin_comment
comment|/**  * _gimp_display_delete: (skip)  * @display_ID: The display to delete.  *  * Delete the specified display.  *  * This procedure removes the specified display. If this is the last  * remaining display for the underlying image, then the image is  * deleted also. Note that the display is closed no matter if the image  * is dirty or not. Better save the image before calling this  * procedure.  *  * Returns: TRUE on success.  **/
end_comment

begin_function
name|gboolean
DECL|function|_gimp_display_delete (gint32 display_ID)
...
begin_comment
comment|/**  * _gimp_displays_reconnect: (skip)  * @old_image_ID: The old image (must have at least one display).  * @new_image_ID: The new image (must not have a display).  *  * Reconnect displays from one image to another image.  *  * This procedure connects all displays of the old_image to the  * new_image. If the old_image has no display or new_image already has  * a display the reconnect is not performed and the procedure returns  * without success. You should rarely need to use this function.  *  * Returns: TRUE on success.  **/
end_comment

begin_function
name|gboolean
DECL|function|_gimp_displays_reconnect (gint32 old_image_ID,gint32 new_image_ID)
