...
condition|)
block|{
comment|/* We can't pass objects over the wire, but we can support        * specific objects which are actually handled in the core through        * IDs by converting these types to their respective IDs.        */
comment|/* Don't compare with libgimp types directly as this file is also        * included from app/ which won't know of the types.        */
if|if
condition|(
name|g_strcmp0
argument_list|(
