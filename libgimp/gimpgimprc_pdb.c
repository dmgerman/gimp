...
begin_comment
comment|/**  * gimp_get_number_of_processors:  *  * Get the number of processors which GIMP was configured to use.  *  * Returns the number of processors which GIMP was configured to use.  * This value is taken from the Preferences and there's no guarantee  * for the value to be reasonable. This function is mainly intended for  * plugin writers who want to write multithreaded plugins and need to  * know how many threads to create.  *  * Returns: The number of processors.  *  * Since: GIMP 2.8  */
end_comment
