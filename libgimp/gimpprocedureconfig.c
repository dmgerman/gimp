...
name|g_printerr
argument_list|(
literal|"Saving last values failed: %s\n"
argument_list|,
name|error
operator|->
name|message
...
begin_comment
comment|/**  * gimp_procedure_config_end_run:  * @config:   a #GimpProcedureConfig  * @run_mode: the #GimpRunMode passed to a #GimpProcedure's run()  *  * This function is the counterpart of  * gimp_procedure_conig_begin_run() and should be used upon successful  * completion of a procedure's run(), before returning  * %GIMP_PDB_SUCCESS return values.  *  * If @run_mode is %GIMP_RUN_INTERACTIVE, @config is saved as last  * values to be used when the procedure runs again.  *  * If @run_mode is not %GIMP_RUN_NONINTERACTIVE, this function also  * conveniently calls gimp_display_flush(), which is what most  * procedures want and doesn't do any harm if called redundantly.  *  * See gimp_procedure_config_begin_run().  *  * Since: 3.0  **/
end_comment

begin_function
name|void
DECL|function|gimp_procedure_config_end_run (GimpProcedureConfig * config,GimpRunMode run_mode)
