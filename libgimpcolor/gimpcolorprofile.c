...
name|GimpColorProfile
modifier|*
DECL|function|gimp_color_profile_new_foobar (GimpColorProfile * profile)
name|gimp_color_profile_new_foobar
parameter_list|(
name|GimpColorProfile
modifier|*
name|profile
parameter_list|)
block|{
comment|/* Make the target RGB working space.  Until the ICC allows other    * illuminants, and corresponding LCMS code has been added, use the    * D50 color space reference white and illuminant.    */
name|GimpColorProfile
modifier|*
name|new_profile
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* I made this profile with the linear gamma TRC, just so it would be    * a little different from the GIMP sRGB profile. There's a way to    * only have one copy of the TRC in the profile, but I haven't figured    * out how to avoid writing three separate but identical curves.    */
name|cmsWriteTag
argument_list|(
name|target_profile
argument_list|,
...
argument_list|,
literal|"Copyright 2015 by GIMP, Creative Commons Attribution-ShareAlike 3.0 Unported License (https://creativecommons.org/licenses/by-sa/3.0/legalcode)."
argument_list|)
expr_stmt|;
name|gimp_color_profile_set_tag
argument_list|(
...
argument_list|,
literal|"GIMP user working space with linear gamma TRC."
argument_list|)
expr_stmt|;
name|gimp_color_profile_set_tag
argument_list|(
