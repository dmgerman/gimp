...
begin_comment
comment|/* gap_pdb_calls.c  *  * this module contains calls of procedures in the GIMPs Procedural Database  *   * IMPORTANT Notes:  *   some Procedures have changed their Interface from GIMP 1.0.2 to GIMP 1.1  *   in that cases the procedure parameters are checked and the call is done  *   in 1.0.2 or 1.1 style respectivly.  *  *   some of these procedures are not available in the official GIMP 1.0.2 releases  *   (and prior releases)  *  *   The missing procedures (except guides) are available as patches to the gimp core.  *   If you dont install the patches GAP will NOT work on GIMP 1.0.2 and older versions.  *  * GIMP 1.1 will provide all the procedures to run GAP at full fuctionality.  * There are no Patches required to run GAP in the latest GIMP 1.1  * development version  */
end_comment
...
begin_comment
comment|/* revision history:  * version 1.1.14a; 2000/01/09  hof: thumbnail save/load,  *                              Procedures for video_info file  * version 0.98.00; 1998/11/28  hof: 1.st (pre) release (GAP port to GIMP 1.1)  */
end_comment
...
name|l_called_proc
argument_list|,
literal|"(cannot save thumbnails)"
argument_list|)
expr_stmt|;
block|}
return|return
...
begin_function
name|gint
DECL|function|p_gimp_file_load_thumbnail (char * filename,gint32 * th_width,gint32 * th_height,unsigned char ** th_data)
...
condition|)
block|{
comment|/* BUG in the PDB Interface in gimp.1.1.14 	    *   the return_vals[3].data.d_int8array does NOT really point 	    *   to the thumbnail data (as it should) 	    *   and we retrieve only NONSENSE DATA here. 	    * 	    *  (maybe we need an additional data length parameter 	    *   as it is usual for array parameter passing ?) 	    */
name|memcpy
argument_list|(
operator|*
name|th_data
...
name|l_called_proc
argument_list|,
literal|"(cannot load thumbnails)"
argument_list|)
expr_stmt|;
block|}
return|return
...
name|l_called_proc
argument_list|,
literal|"(cannot get image thumbnail)"
argument_list|)
expr_stmt|;
block|}
return|return
