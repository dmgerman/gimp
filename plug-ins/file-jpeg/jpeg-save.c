...
condition|)
block|{
comment|/* When we don't save profiles, we convert data to sRGB because        * that's what most/all readers expect on a no-profile JPEG.        * If we save an assigned profile, let's just follow its TRC.        * If we save the default linear profile (i.e. no assigned        * profile), we convert it to sRGB.        */
if|if
condition|(
name|profile
operator|&&
...
expr_stmt|;
block|}
comment|/* Step 4.2: store the color profile */
if|if
condition|(
name|jsvals
operator|.
...
condition|)
block|{
comment|/* Convert profile from linear to sRGB. This would normally            * only happen when there was no assigned profile (i.e. the            * default linear profile is in use). */
name|GimpColorProfile
modifier|*
name|saved_profile
decl_stmt|;
