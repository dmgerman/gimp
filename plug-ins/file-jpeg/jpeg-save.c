...
return|;
block|}
comment|/* When we don't save profiles, we convert data to sRGB because    * that's what most/all readers expect on a no-profile JPEG.    * If we save an assigned profile, let's just follow its TRC.    * If we save the default linear profile (i.e. no assigned    * profile), we convert it to sRGB, except when it is 8-bit linear.    */
if|if
condition|(
name|jsvals
operator|.
