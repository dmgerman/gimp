...
typedef|typedef
struct|struct
DECL|struct|__anon2c2872ad0108
block|{
DECL|member|cinfo
name|struct
...
typedef|typedef
struct|struct
DECL|struct|__anon2c2872ad0208
block|{
DECL|member|run
name|gboolean
name|run
...
condition|)
block|{
comment|/* In most cases we want to save 8-bit image formats as        * non-linear, even though the work image may have been linear        * itself (yet with higher bit depth). The reasons are shadow        * posterization on low bit depth, and the fact that JPEG        * compression was designed for perceptually uniform RGB and        * introduces shadow artifacts with linear RGB (or so I have been        * told; see #1070).        * The only exception is when the creator was working explicitly        * on 8-bit linear AND if we export the profile.        * In such a case, let's consider the creator knows what one is        * doing and keep the exported image linear.        */
if|if
condition|(
...
case|:
comment|/* Convert profile from linear to sRGB. */
block|{
name|GimpColorProfile
modifier|*
name|saved_profile
...
name|GIMP_PRECISION_U8_LINEAR
case|:
comment|/* Only exception where we don't convert from linear to                * sRGB TRC is when the work format was 8-bit linear. */
case|case
...
name|def_str
argument_list|,
literal|"%lf %lf %d %d %d %d %d %d %d %d %d %d %d %d %d %d"
argument_list|,
operator|&
name|tmpvals
operator|.
