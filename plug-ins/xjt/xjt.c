...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00108
block|{
DECL|member|run
name|gint
name|run
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00808
block|{
DECL|member|prop_id
name|t_proptype
name|prop_id
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00908
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00a08
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00b08
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00c08
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00d08
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00e08
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c270ec00f08
block|{
DECL|member|version
name|gchar
modifier|*
...
name|_
argument_list|(
literal|"Error: Cant read XJT propertyfile %s"
argument_list|)
argument_list|,
name|prop_filename
argument_list|)
...
comment|/* hof: 2000.01.30     * The  PDB-interface of     *   gimp_path_set_points gimp 1.1.15     *   --------------------     *   does not work, especially if the image has no display.     *   Most of the time it crashed and     *   in some cases cases it just brings unusable results.     *     * for debug reasons i added a display     *     *   but (as usual in file load plugins), the gimp     *   main app opens a new display at success     *   and our image gets 2 displays     *   -- so this is not a usable workaround     *   If somebody can fix the problem until gimp 1.2     *   we can remove the ugly WARNING printf's     *   and the call to     *      gimp_display_new(l_image_id);     */
if|if
condition|(
name|l_image_prp_ptr
operator|->
...
name|printf
argument_list|(
literal|"XJT: WARNING paths found in file %s were not attached to the image\n"
argument_list|,
name|filename
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"     because the PDB interface of gimp_path_set_points does not work in gimp 1.1.15\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"     you may try to set the XJT_DEBUG Environmentvariable and restart the gimp\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"       XJT_DEBUG=1    ... try workaround (results in opening of 2 displays)\n"
argument_list|)
expr_stmt|;
name|printf
argument_list|(
literal|"       XJT_DEBUG=2    ... try the (buggy) call without workaround, and test the crash\n"
argument_list|)
expr_stmt|;
comment|/* activate the next line when the Bug in gimp_path_set_points is fixed */
comment|/* p_add_paths(l_image_id,  l_image_prp_ptr->path_props); */
block|}
block|}
name|cleanup
label|:
