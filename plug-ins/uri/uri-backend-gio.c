...
directive|if
literal|0
block|if (! success&&       run_mode == GIMP_RUN_INTERACTIVE&&       (*error)->domain == G_IO_ERROR&&       (*error)->code   == G_IO_ERROR_NOT_MOUNTED)     {       if (mount_enclosing_volume (mode == DOWNLOAD ? src_file : dest_file,                                   error))         {           success = g_file_copy (src_file, dest_file, 0, NULL,                                  uri_progress_callback, GINT_TO_POINTER (mode),                                  error);          }     }
endif|#
directive|endif
name|g_object_unref
argument_list|(
