...
directive|if
literal|0
block|if (vars.left< 0 || vars.top< 0)   {     vars.left = (page_width - print_width) / 2;     vars.top  = (page_height - print_height) / 2;   };    vars.left += event->x - mouse_x;   vars.top  += event->y - mouse_y;    if (vars.left< 0)     vars.left = 0;    if (vars.top< 0)     vars.top = 0;    preview_update();    mouse_x = event->x;   mouse_y = event->y;
endif|#
directive|endif
comment|/* 0 */
block|}
