...
begin_comment
comment|/*    *   First, you should not use error_t since this is already defined   *       in some system headers; changed it to dither_error_t.  *   Second, this code is unused, so I have commented it out.  *                                2000-04-28  Sven Neumann<sven@gimp.org>  */
end_comment

begin_comment
comment|/*   #define ERROR_ROWS 2    typedef union error   {     struct     {       int c[ERROR_ROWS];       int m[ERROR_ROWS];       int y[ERROR_ROWS];       int k[ERROR_ROWS];     } c;     int v[4][ERROR_ROWS];   } dither_error_t;    dither_error_t *nerror = 0; */
end_comment
...
parameter_list|)
define|\
value|do {									    \   int comp0 = (32767 + ((ditherbit##d2>> R##_RANDOMIZER) -		    \ 			(32768>> R##_RANDOMIZER)));			    \   if (!l##color)							    \     {									    \       if (r> comp0)							    \ 	{								    \ 	  PRINT_D1(r, R, d1, d2);					    \ 	  DO_PRINT_COLOR(r);						    \ 	  r -= 65536;							    \ 	}								    \     }									    \   else									    \     {									    \       int compare = comp0 * I_RATIO_##R##1;				    \       if (r<= (R##_CONST_1))						    \ 	{								    \ 	  if (r> compare)						    \ 	    {								    \ 	      PRINT_D2(r, R, d1, d2);					    \ 	      DO_PRINT_COLOR(l##r);					    \ 	      r -= R##_CONST_0;						    \ 	    }								    \ 	}								    \       else if (r> compare)						    \ 	{								    \ 	  int cutoff = ((density - R##_CONST_1) * 65536 /		    \ 			(65536 - R##_CONST_1));				    \ 	  long long sub;						    \ 	  if (cutoff>= 0)						    \ 	    sub = R##_CONST_0 + (((65535ll - R##_CONST_0) * cutoff)>> 16); \ 	  else								    \ 	    sub = R##_CONST_0 + ((65535ll - R##_CONST_0) * cutoff / 65536); \ 	  if (ditherbit##d1> cutoff)					    \ 	    {								    \ 	      PRINT_D3(3, r, R, d1, d2);				    \ 	      DO_PRINT_COLOR(l##r);					    \ 	    }								    \ 	  else								    \ 	    {								    \ 	      PRINT_D3(4, r, R, d1, d2);				    \ 	      DO_PRINT_COLOR(r);					    \ 	    }								    \ 	  if (sub< R##_CONST_0)					    \ 	    r -= R##_CONST_0;						    \ 	  else if (sub> 65535)						    \ 	    r -= 65536;							    \ 	  else								    \ 	    r -= sub;							    \ 	}								    \     }									    \ } while (0)
end_define
...
parameter_list|)
define|\
value|do {									   \   int offset = ((15 - (((o##r& 0xf000)>> 12))) * horizontal_overdensity) \>> 1;				   \   int tmp##r = r;							   \   if (tmp##r> 65535)							   \     tmp##r = 65535;							   \   if (x< offset)							   \     offset = x;								   \   else if (x> dst_width - offset - 1)					   \     offset = dst_width - x - 1;						   \   if (ditherbit##d2& bit)						   \     {									   \       r##error1[-offset] += tmp##r;					   \       r##error1[0] += 3 * tmp##r;					   \       r##error1[offset] += tmp##r;					   \       if (x> 0&& x< (dst_width - 1))					   \ 	dither##r    = r##error0[direction] + 3 * tmp##r;		   \     }									   \   else									   \     {									   \       r##error1[-offset] += tmp##r;					   \       r##error1[0] +=  tmp##r;						   \       r##error1[offset] += tmp##r;					   \       if (x> 0&& x< (dst_width - 1))					   \ 	dither##r    = r##error0[direction] + 5 * tmp##r;		   \     }									   \   } while (0)
end_define
...
begin_comment
comment|/* The version in libgimp IS NOT SUITABLE FOR THIS TASK.  That version is */
end_comment

begin_comment
comment|/* 8 bits.  We need the 16 bit version! */
end_comment
