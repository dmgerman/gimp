...
name|BOOL
name|CALLBACK
DECL|function|GetAccurateWindowRect_MonitorEnumProc (HMONITOR hMonitor,HDC hdcMonitor,LPRECT lprcMonitor,LPARAM dwData)
name|GetAccurateWindowRect_MonitorEnumProc
parameter_list|(
name|HMONITOR
name|hMonitor
parameter_list|,
...
specifier|static
name|BOOL
DECL|function|GetAccurateWindowRect (HWND hwndTarget,RECT * outRect)
name|GetAccurateWindowRect
parameter_list|(
name|HWND
name|hwndTarget
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* First we try to get the rect using the dwm api. it will also avoid us from doing more ugly fixes when the window is maximized */
if|if
condition|(
...
return|;
block|}
comment|/* In this case, we did not got the rect from the dwm api so we try to get the rect with the normal function */
if|if
condition|(
name|GetWindowRect
argument_list|(
