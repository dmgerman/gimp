...
return|;
block|}
comment|/* with an ftp url wget has a "=> `filename.foo" */
elseif|else
if|if
condition|(
operator|!
...
return|;
block|}
comment|/* on successful ftp login wget prints a "Logged in" message */
elseif|else
if|if
condition|(
name|seen_ftp
...
argument_list|)
expr_stmt|;
comment|/* for an ftp session wget has extra output*/
name|ftp_session
label|:
if|if
condition|(
...
return|;
block|}
comment|/* if there is no size output file does not exist on server */
elseif|else
if|if
condition|(
name|strstr
...
return|;
block|}
comment|/* while no PASV line we eat other messages */
elseif|else
if|if
condition|(
operator|!
...
expr_stmt|;
block|}
comment|/* on http sessions wget has "Saving to: ..." */
if|if
condition|(
operator|!
name|seen_ftp
