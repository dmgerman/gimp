...
directive|define
name|ECHO
value|do { if (fwrite( ncsa_text, ncsa_leng, 1, ncsa_out )) {} } while (0)
end_define
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in ncsa_ensure_buffer_stack()"
argument_list|)
expr_stmt|;
name|memset
argument_list|(
...
begin_function
DECL|function|ncsa__scan_string (yyconst char * yystr)
name|YY_BUFFER_STATE
name|ncsa__scan_string
parameter_list|(
name|yyconst
...
begin_function
DECL|function|ncsa__scan_bytes (yyconst char * yybytes,int _yybytes_len)
name|YY_BUFFER_STATE
name|ncsa__scan_bytes
parameter_list|(
name|yyconst
...
parameter_list|)
block|{
comment|/* Initialization is the same as for the non-reentrant scanner.      * This function is called from ncsa_lex_destroy(), so don't allocate here.      */
operator|(
name|yy_buffer_stack
operator|)
operator|=
...
directive|endif
comment|/* For future reference: Set errno on error, since we are called by      * ncsa_lex_init()      */
return|return
literal|0
return|;
block|}
...
name|NULL
expr_stmt|;
comment|/* Reset the globals. This is important in a non-reentrant scanner so the next time      * ncsa_lex() is called, initialization will occur. */
name|yy_init_globals
argument_list|( )
expr_stmt|;
return|return
