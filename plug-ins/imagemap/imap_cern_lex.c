...
define|\
value|do \ 		{ \
comment|/* Undo effects of setting up cern_text. */
...
comment|/* set up cern_text again */
value|\ 		} \ 	while ( 0 )
end_define
...
begin_comment
comment|/* yy_hold_char holds the character lost when cern_text is formed. */
end_comment
...
begin_comment
comment|/* Flag which is used to allow cern_wrap()'s to do buffer switches  * instead of setting up a fresh cern_in.  A bit of a hack ...  */
end_comment
...
directive|define
name|YY_FLUSH_BUFFER
value|cern__flush_buffer(YY_CURRENT_BUFFER )
end_define

begin_function_decl
name|YY_BUFFER_STATE
name|cern__scan_buffer
parameter_list|(
name|char
modifier|*
name|base
...
parameter_list|)
define|\
value|{ \ 	if ( ! YY_CURRENT_BUFFER ){ \         cern_ensure_buffer_stack (); \ 		YY_CURRENT_BUFFER_LVALUE =    \             cern__create_buffer(cern_in,YY_BUF_SIZE ); \ 	} \ 	YY_CURRENT_BUFFER_LVALUE->yy_is_interactive = is_interactive; \ 	}
end_define
...
parameter_list|)
define|\
value|{ \ 	if ( ! YY_CURRENT_BUFFER ){\         cern_ensure_buffer_stack (); \ 		YY_CURRENT_BUFFER_LVALUE =    \             cern__create_buffer(cern_in,YY_BUF_SIZE ); \ 	} \ 	YY_CURRENT_BUFFER_LVALUE->yy_at_bol = at_bol; \ 	}
end_define
...
begin_decl_stmt
DECL|variable|cern_in
DECL|variable|cern_out
name|FILE
modifier|*
name|cern_in
init|=
...
begin_comment
comment|/* Done after the current pattern has been matched and before the  * corresponding action - sets up cern_text.  */
end_comment
...
define|\
value|(yytext_ptr) = yy_bp; \ 	cern_leng = (size_t) (yy_cp - yy_bp); \ 	(yy_hold_char) = *yy_cp; \ 	*yy_cp = '\0'; \ 	(yy_c_buf_p) = yy_cp;
end_define
...
specifier|extern
name|int
name|cern__flex_debug
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|cern__flex_debug
name|int
name|cern__flex_debug
init|=
literal|0
...
directive|define
name|ECHO
value|do { if (fwrite( cern_text, cern_leng, 1, cern_out )) {} } while (0)
end_define
...
begin_comment
comment|/* Code executed at the beginning of each rule, after cern_text and cern_leng  * have been set up.  */
end_comment
...
condition|)
block|{
comment|/* Note: because we've taken care in 					 * yy_get_next_buffer() to have set up 					 * cern_text, we can now set up 					 * yy_c_buf_p so that if some total 					 * hoser (like flex itself) wants to 					 * call the scanner after we return the 					 * YY_NULL, it'll still work - another 					 * YY_NULL will get returned. 					 */
operator|(
name|yy_c_buf_p
operator|)
operator|=
...
begin_function
DECL|function|cern_restart (FILE * input_file)
name|void
name|cern_restart
parameter_list|(
name|FILE
...
begin_function
DECL|function|cern__switch_to_buffer (YY_BUFFER_STATE new_buffer)
name|void
name|cern__switch_to_buffer
parameter_list|(
name|YY_BUFFER_STATE
...
parameter_list|)
block|{
comment|/* TODO. We should be able to replace this entire function body 	 * with 	 *		cern_pop_buffer_state(); 	 *		cern_push_buffer_state(new_buffer);      */
name|cern_ensure_buffer_stack
argument_list|()
expr_stmt|;
if|if
...
argument_list|( )
expr_stmt|;
comment|/* We don't actually know whether we did this switch during 	 * EOF (cern_wrap()) processing, but the only time this flag 	 * is looked at is after cern_wrap() is called, so it's safe 	 * to go ahead and always set it. 	 */
operator|(
name|yy_did_buffer_switch_on_eof
operator|)
operator|=
...
begin_function
DECL|function|cern__load_buffer_state (void)
specifier|static
name|void
name|cern__load_buffer_state
parameter_list|(
...
begin_function
DECL|function|cern__create_buffer (FILE * file,int size)
name|YY_BUFFER_STATE
name|cern__create_buffer
parameter_list|(
name|FILE
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in cern__create_buffer()"
argument_list|)
expr_stmt|;
name|b
operator|->
...
begin_comment
comment|/** Destroy the buffer.  * @param b a buffer created with cern__create_buffer()  *   */
end_comment

begin_function
DECL|function|cern__delete_buffer (YY_BUFFER_STATE b)
name|void
name|cern__delete_buffer
parameter_list|(
name|YY_BUFFER_STATE
...
begin_function
DECL|function|cern__init_buffer (YY_BUFFER_STATE b,FILE * file)
specifier|static
name|void
name|cern__init_buffer
parameter_list|(
...
literal|1
expr_stmt|;
comment|/* If b is the current buffer, then cern__init_buffer was _probably_      * called from cern_restart() or through yy_get_next_buffer.      * In that case, we don't want to reset the lineno or column.      */
if|if
condition|(
name|b
operator|!=
...
begin_function
DECL|function|cern__flush_buffer (YY_BUFFER_STATE b)
name|void
name|cern__flush_buffer
parameter_list|(
name|YY_BUFFER_STATE
...
begin_function
DECL|function|cern_push_buffer_state (YY_BUFFER_STATE new_buffer)
name|void
name|cern_push_buffer_state
parameter_list|(
name|YY_BUFFER_STATE
...
argument_list|()
expr_stmt|;
comment|/* This block is copied from cern__switch_to_buffer. */
if|if
condition|(
name|YY_CURRENT_BUFFER
condition|)
...
name|new_buffer
expr_stmt|;
comment|/* copied from cern__switch_to_buffer. */
name|cern__load_buffer_state
argument_list|( )
expr_stmt|;
operator|(
...
begin_function
DECL|function|cern_pop_buffer_state (void)
name|void
name|cern_pop_buffer_state
parameter_list|(
name|void
...
begin_function
DECL|function|cern_ensure_buffer_stack (void)
specifier|static
name|void
name|cern_ensure_buffer_stack
parameter_list|(
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in cern_ensure_buffer_stack()"
argument_list|)
expr_stmt|;
name|memset
argument_list|(
...
begin_function
DECL|function|cern__scan_buffer (char * base,yy_size_t size)
name|YY_BUFFER_STATE
name|cern__scan_buffer
parameter_list|(
name|char
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in cern__scan_buffer()"
argument_list|)
expr_stmt|;
name|b
operator|->
...
begin_comment
comment|/** Setup the input buffer state to scan a string. The next call to cern_lex() will  * scan from a @e copy of @a str.  * @param yystr a NUL-terminated string to scan  *   * @return the newly allocated buffer state object.  * @note If you want to scan bytes that may contain NUL values, then use  *       cern__scan_bytes() instead.  */
end_comment

begin_function
DECL|function|cern__scan_string (yyconst char * yystr)
name|YY_BUFFER_STATE
name|cern__scan_string
parameter_list|(
name|yyconst
...
name|YY_FATAL_ERROR
argument_list|(
literal|"out of dynamic memory in cern__scan_bytes()"
argument_list|)
expr_stmt|;
for|for
control|(
...
name|YY_FATAL_ERROR
argument_list|(
literal|"bad buffer in cern__scan_bytes()"
argument_list|)
expr_stmt|;
...
value|do \ 		{ \
comment|/* Undo effects of setting up cern_text. */
value|\         int yyless_macro_arg = (n); \         YY_LESS_LINENO(yyless_macro_arg);\ 		cern_text[cern_leng] = (yy_hold_char); \ 		(yy_c_buf_p) = cern_text + yyless_macro_arg; \ 		(yy_hold_char) = *(yy_c_buf_p); \ 		*(yy_c_buf_p) = '\0'; \ 		cern_leng = yyless_macro_arg; \ 		} \ 	while ( 0 )
end_define
...
begin_function
DECL|function|cern_get_lineno (void)
name|int
name|cern_get_lineno
parameter_list|(
name|void
...
begin_function
DECL|function|cern_get_in (void)
name|FILE
modifier|*
name|cern_get_in
parameter_list|(
...
begin_function
DECL|function|cern_get_out (void)
name|FILE
modifier|*
name|cern_get_out
parameter_list|(
...
begin_function
DECL|function|cern_get_text (void)
name|char
modifier|*
name|cern_get_text
parameter_list|(
...
begin_function
DECL|function|cern_set_lineno (int line_number)
name|void
name|cern_set_lineno
parameter_list|(
name|int
...
begin_comment
comment|/** Set the input stream. This does not discard the current  * input buffer.  * @param in_str A readable stream.  *   * @see cern__switch_to_buffer  */
end_comment

begin_function
DECL|function|cern_set_in (FILE * in_str)
name|void
name|cern_set_in
parameter_list|(
name|FILE
...
begin_function
DECL|function|cern_set_out (FILE * out_str)
name|void
name|cern_set_out
parameter_list|(
name|FILE
...
begin_function
DECL|function|cern_get_debug (void)
name|int
name|cern_get_debug
parameter_list|(
name|void
...
begin_function
DECL|function|cern_set_debug (int bdebug)
name|void
name|cern_set_debug
parameter_list|(
name|int
...
parameter_list|)
block|{
comment|/* Initialization is the same as for the non-reentrant scanner.      * This function is called from cern_lex_destroy(), so don't allocate here.      */
operator|(
name|yy_buffer_stack
operator|)
operator|=
...
directive|endif
comment|/* For future reference: Set errno on error, since we are called by      * cern_lex_init()      */
return|return
literal|0
return|;
block|}
...
begin_comment
comment|/* cern_lex_destroy is for both reentrant and non-reentrant scanners. */
end_comment

begin_function
DECL|function|cern_lex_destroy (void)
name|int
name|cern_lex_destroy
parameter_list|(
name|void
...
name|NULL
expr_stmt|;
comment|/* Reset the globals. This is important in a non-reentrant scanner so the next time      * cern_lex() is called, initialization will occur. */
name|yy_init_globals
argument_list|( )
expr_stmt|;
return|return
...
begin_function
DECL|function|cern_alloc (yy_size_t size)
name|void
modifier|*
name|cern_alloc
parameter_list|(
...
begin_function
DECL|function|cern_realloc (void * ptr,yy_size_t size)
name|void
modifier|*
name|cern_realloc
parameter_list|(
...
begin_function
DECL|function|cern_free (void * ptr)
name|void
name|cern_free
parameter_list|(
name|void
...
argument_list|)
expr_stmt|;
comment|/* see cern_realloc() for (char *) cast */
block|}
end_function
