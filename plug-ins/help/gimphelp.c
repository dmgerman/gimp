...
expr_stmt|;
block|}
comment|/*  error message that need to be added back;       stored here to avoid string changes        */
if|#
directive|if
literal|0
block|if (! strcmp (domain->help_domain, GIMP_HELP_DEFAULT_DOMAIN))     msg = _("The GIMP help files are not installed.");   else     msg = _("The requested help files are not installed.");    msg2 = g_strdup_printf (_("Could not open '%s' for reading: %s"),                           gimp_filename_to_utf8 (filename),                           (*error)->message);    g_clear_error (error);    g_set_error (error, 0, 0, "%s\n\n%s\n\n%s",                msg, msg2, _("Please check your installation."));    if (! domain->help_id_mapping)     g_message (_("Failed to open help files:\n%s"), error->message);   else     g_message (_("Parse error in help domain:\n%s\n\n"                  "(entries appearing before the error have been added anyway)"),                error->message);
endif|#
directive|endif
if|if
condition|(
...
typedef|typedef
struct|struct
DECL|struct|__anon2779ad7c0208
block|{
