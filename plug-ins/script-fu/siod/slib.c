...
begin_function_decl
DECL|variable|siod_output_routine
name|void
function_decl|(
modifier|*
...
literal|'_'
case|:
comment|/*  might be a string marked for translation using _(...)  */
name|c
operator|=
name|GETC_FCN
argument_list|(
...
directive|if
literal|0
block|else if ((fname[0] != '/')&& 	       ((strlen (siod_lib) + strlen (fname) + 1)< sizeof (buffer))) 	{ 	  strcpy (buffer, siod_lib); 	  strcat (buffer, "/"); 	  strcat (buffer, fname); 	  if ((f = g_fopen (buffer, "r"))) 	    { 	      fname = buffer; 	      fclose (f); 	    } 	}
endif|#
directive|endif
name|no_interrupt
argument_list|(
