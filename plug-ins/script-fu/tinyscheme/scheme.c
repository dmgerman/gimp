...
directive|define
name|banner
value|"TinyScheme 1.41 (with UTF-8 support)"
end_define
...
argument_list|)
expr_stmt|;
comment|/* Mark any older stuff above nested C calls */
name|mark
argument_list|(
name|sc
operator|->
...
directive|if
name|SHOW_ERROR_LINE
comment|/* Cleanup is here so (close-*-port) functions could work too */
name|pt
operator|->
name|rep
operator|.
...
name|c
return|;
comment|/* Is this byte an invalid lead per RFC-3629? */
if|if
condition|(
name|c
operator|<
...
argument_list|)
expr_stmt|;
comment|/* Stop reading if this is not a continuation character */
if|if
condition|(
operator|(
name|c
...
name|len
condition|)
comment|/* Read the expected number of bytes? */
block|{
return|return
...
return|;
block|}
comment|/* Not enough continuation characters so ignore and restart */
block|}
block|}
...
condition|)
block|{
comment|/* Found NUL or at end of input buffer? */
if|if
condition|(
operator|*
name|pt
...
argument_list|)
expr_stmt|;
comment|/* Length of UTF-8 sequence */
name|pt
operator|->
name|rep
operator|.
...
return|;
block|}
comment|/* Look for next valid UTF-8 character in buffer */
name|pt
operator|->
name|rep
operator|.
...
condition|)
block|{
comment|/* Instead, set port_saw_EOF */
name|pt
operator|->
name|kind
operator||=
...
literal|10
condition|)
comment|/* f is the base for numbers if> 1 */
block|{
if|if
condition|(
name|num_is_integer
...
begin_function
DECL|function|revappend (scheme * sc,pointer a,pointer b)
specifier|static
name|pointer
name|revappend
parameter_list|(
...
index|]
decl_stmt|;
comment|/* make sure error is not in REPL */
if|if
condition|(
name|sc
operator|->
...
case|:
comment|/* top level */
comment|/* If we reached the end of file, this loop is done. */
if|if
condition|(
name|sc
operator|->
...
comment|/* NOTREACHED */
block|}
comment|/* If interactive, be nice to user. */
if|if
condition|(
name|file_interactive
argument_list|(
...
expr_stmt|;
block|}
comment|/* Set up another iteration of REPL */
name|sc
operator|->
name|nesting
operator|=
...
expr_stmt|;
block|}
comment|/* Before returning integer result make sure we can. */
comment|/* If the test fails, result is too big for integer. */
if|if
condition|(
operator|!
name|real_result
...
condition|)
block|{
comment|/* we know cadr(sc->args) is a natural number */
comment|/* see if it is 2, 8, 10, or 16, or error */
name|pf
operator|=
name|ivalue_unchecked
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Safe because we would have already returned if `fast'           encountered a non-pair. */
name|slow
operator|=
name|cdr
argument_list|(
...
expr_stmt|;
block|}
comment|/* cdr() in the while condition is not a typo. If car() */
comment|/* is used (append '() 'a) will return the wrong result.*/
while|while
condition|(
name|cdr
argument_list|(
...
name|sc
argument_list|,
literal|"non-list argument to append"
argument_list|)
expr_stmt|;
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|/* Truncate the dump stack so TS will return here when done, not      directly resume pre-C-call operations. */
name|dump_stack_reset
argument_list|(
name|sc
argument_list|)
