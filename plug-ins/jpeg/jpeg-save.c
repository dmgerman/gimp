...
typedef|typedef
struct|struct
DECL|struct|__anon27d945d90108
block|{
DECL|member|quality
name|gdouble
name|quality
...
typedef|typedef
struct|struct
DECL|struct|__anon27d945d90208
block|{
DECL|member|run
name|gint
name|run
...
directive|ifdef
name|GIMP_HAVE_PARASITES
comment|/* pw - Walk the marker list looking for comments (that is the only    * thing that should be in there - but be explicit.)  If there are    * multiple comments, merge them into one with a /n between each. */
for|for
control|(
name|curr_marker
operator|=
...
name|g_warning
argument_list|(
literal|"*** Warning *** comment truncated\n"
argument_list|)
expr_stmt|;
block|}
if|if
