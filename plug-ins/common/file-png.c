...
typedef|typedef
struct|struct
DECL|struct|__anon2c275c340108
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2c275c340208
block|{
DECL|member|run
name|gboolean
name|run
...
typedef|typedef
struct|struct
DECL|struct|__anon2c275c340308
block|{
...
directive|if
literal|0
block|case PNG_COLOR_TYPE_PALETTE:
comment|/* Indexed */
block|bpp = 1;       image_type = GIMP_INDEXED;       layer_type = GIMP_INDEXED_IMAGE;       break;
endif|#
directive|endif
default|default:
...
directive|if
literal|0
block|if (trns)     {       gimp_layer_add_alpha (layer);       drawable = gimp_drawable_get (layer);       gimp_pixel_rgn_init (&pixel_rgn, drawable, 0, 0, drawable->width,                            drawable->height, TRUE, FALSE);        pixel = g_new (guchar, tile_height * drawable->width * 2);
comment|/* bpp == 1 */
block|for (begin = 0, end = tile_height;            begin< drawable->height; begin += tile_height, end += tile_height)         {           if (end> drawable->height)             end = drawable->height;           num = end - begin;            gimp_pixel_rgn_get_rect (&pixel_rgn, pixel, 0, begin,                                    drawable->width, num);            for (i = 0; i< tile_height * drawable->width; ++i)             {               pixel[i * 2 + 1] = alpha[pixel[i * 2]];               pixel[i * 2] -= empty;             }            gimp_pixel_rgn_set_rect (&pixel_rgn, pixel, 0, begin,                                    drawable->width, num);         }        g_free (pixel);     }
endif|#
directive|endif
return|return
name|image
...
name|file_format
decl_stmt|;
comment|/* BABL format of drawable */
name|png_structp
name|pp
decl_stmt|;
comment|/* PNG read pointer */
...
directive|if
literal|0
comment|/* indexed missing */
block|case GIMP_INDEXED_IMAGE:       bpp = 1;       color_type = PNG_COLOR_TYPE_PALETTE;       pngg.has_plte = TRUE;       pngg.palette = (png_colorp) gimp_image_get_colormap (image_ID,&pngg.num_palette);       bit_depth = get_bit_depth_for_palette (pngg.num_palette);       break;      case GIMP_INDEXEDA_IMAGE:       bpp = 2;       color_type = PNG_COLOR_TYPE_PALETTE;
comment|/* fix up transparency */
block|bit_depth = respin_cmap (pp, info, remap, image_ID, drawable);       break;
endif|#
directive|endif
default|default:
name|g_set_error
