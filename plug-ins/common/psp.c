...
begin_comment
comment|/*  *  * Work in progress! Doesn't do anything at all useful yet.  *  * For a copy of the PSP file format documentation, surf to  * http://www.jasc.com.  *  */
end_comment
...
begin_comment
comment|/* The following was cut and pasted from the PSP file format  * documentation version 3.0.(Minor stylistic changes done.)  *  * Note that the upcoming PSP version 6 writes PSP file format version  * 4.0, but the documentation for that apparently isn't publicly  * available (yet). The format is designed to be downward compatible,  * however. The semantics of many of the additional fields and  * block types can be deduced by reverse engineering.  */
end_comment
...
typedef|typedef
struct|struct
DECL|struct|__anon28b156f90d08
block|{
DECL|member|width
DECL|member|height
name|guint32
...
typedef|typedef
struct|struct
DECL|struct|__anon28b156f90e08
block|{
DECL|member|compression
name|PSPCompression
name|compression
...
typedef|typedef
struct|struct
DECL|struct|__anon28b156f90f08
block|{
DECL|member|run
name|gint
name|run
...
specifier|static
name|void
DECL|function|read_channel_data (FILE * f,PSPCompression compression,guchar ** pixels,guint bpp,GDrawable * drawable,GPixelRgn * pixel_rgn,PSPDIBType bitmap_type,PSPChannelType channel_type,guint32 compressed_len)
...
argument_list|(
operator|(
literal|"PSP: layer: %s %dx%d (%dx%d) opacity %d blend_mode %s "
