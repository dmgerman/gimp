...
typedef|typedef
struct|struct
DECL|struct|__anon28f4a1e60108
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon28f4a1e60208
block|{
DECL|member|run
name|gboolean
name|run
...
decl_stmt|;
comment|/* Pixel data */
comment|/* b&w needs this for bits*/
name|guchar
modifier|*
name|bits
init|=
...
name|NULL
decl_stmt|;
comment|/* pointers to BOR */
name|gint
name|bit_line_len
init|=
literal|0
decl_stmt|;
comment|/* number of bytes to keep the bits */
name|guchar
modifier|*
name|bit2byte
init|=
...
condition|)
block|{
comment|/* this is a (maybe useless) attempt to recover cleanly from errors. */
if|#
directive|if
name|PNG_LIBPNG_VER
operator|>
...
condition|)
block|{
comment|/* b&w, load this as an indexed image */
name|is_bw
operator|=
name|TRUE
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* mmc: this means, that png_set_expand& png_set_packing may change info->color_type? */
switch|switch
condition|(
name|info
operator|->
...
case|:
comment|/* Grayscale */
comment|/* g_print("opening a grayscale png. bit depth: %d\n", info->bit_depth); */
if|if
condition|(
name|is_bw
condition|)
block|{
comment|/* Have to construct the white&black  palette, see below. */
name|image_type
operator|=
name|GIMP_INDEXED
expr_stmt|;
...
literal|1
expr_stmt|;
comment|/* This is fake, we disable the expansion (by libpng)!                      * But, after our conversion the rows will indeed be 1 byte-per-pixel. */
name|bit2byte
operator|=
name|fill_bit2byte
argument_list|()
...
condition|)
block|{
comment|/* we are converting grayscale image to a b&w palette: this is the palette then: */
name|guchar
name|bw_map
index|[]
init|=
...
name|i
expr_stmt|;
comment|/* g_print("bit_line_len =%d\n", bit_line_len); */
block|}
for|for
control|(
name|i
...
argument_list|)
expr_stmt|;
comment|/* mmc: interlacing not handled now? */
comment|/* if we used the libpng builtin expansion, we would have to            * convert values 255 (black) into 1 (palette index)            * I think this approach is faster:            */
if|if
condition|(
name|is_bw
condition|)
...
argument_list|)
expr_stmt|;
comment|/* FIXME: we must not memcpy after the boundary!                * So, if the line is not a multiple of 8 (bits per byte),                * we should do it out of the cycle!*/
...
comment|/* indexed b/w -> save as 1bit grayscale to save space! */
name|gboolean
name|is_bw
init|=
name|FALSE
...
condition|)
block|{
comment|/* mmc: fixme: neo says, on failure a saving process exits, so this might be done implicitely         * I do it explicitely for case someone reuses the code. */
if|#
directive|if
name|PNG_LIBPNG_VER
operator|>
...
condition|)
block|{
comment|/* indexed b/w -> 1-grayscale: at this stage, */
comment|/* we have to convert the 0/1 indexes(bytes) into 0/1 bits */
comment|/* pixel   is the array of several lines of the image: indexes                 * pixels  is an array of pointers at the beginnings of the lines */
for|for
control|(
name|i
operator|=
