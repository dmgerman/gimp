...
typedef|typedef
struct|struct
DECL|struct|__anon2a092baa0108
block|{
...
typedef|typedef
struct|struct
DECL|struct|__anon2a092baa0208
block|{
DECL|member|ID
name|gint32
name|ID
...
typedef|typedef
struct|struct
DECL|struct|__anon2a092baa0308
block|{
DECL|member|o_pages
name|gint
name|o_pages
...
begin_endif
unit|static void fill_bit2byte(void) {   static gboolean filled = FALSE;    guchar *dest;   gint    i, j;    if (filled)     return;    dest = bit2byte;    for (j = 0; j< 256; j++)     for (i = 7; i>= 0; i--)       *(dest++) = ((j& (1<< i)) != 0);    filled = TRUE; }
endif|#
directive|endif
end_endif
